...
# Convert to datetime for filtering - use UTC to avoid tz-aware/naive mismatch
...
# Parse POS timestamp and convert to UTC
...
# Parse inventory week_starting and convert to UTC
...
top_item_name = sku_metrics.iloc[0]['item_name_en'] if len(sku_metrics) > 0 else "N/A"
top_item_profit = sku_metrics.iloc[0]['gross_profit_sar'] if len(sku_metrics) > 0 else 0

finding_1 = {
    "title": "Item-Level Gross Profit and Margin Analysis (Analysis Week)",
    "claim": f"During the analysis week (2026-07-20 to 2026-07-27), total gross profit was {total_gross_profit:.2f} SAR across {unique_transactions} transactions with {total_gross_margin:.2f}% gross margin. Top performer: {top_item_name} with {top_item_profit:.2f} SAR profit.",
...
# Parse email dates - convert to UTC
emails_df['date_dt'] = pd.to_datetime(emails_df['date'], utc=True)
emails_df['effective_date_dt'] = pd.to_datetime(emails_df['effective_date'], utc=True)
