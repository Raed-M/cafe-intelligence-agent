...
# Make comparison datetimes timezone-aware (UTC+3)
analysis_start_tz = pd.Timestamp(analysis_start, tz='UTC+03:00')
analysis_end_tz = pd.Timestamp(analysis_end, tz='UTC+03:00')
previous_start_tz = pd.Timestamp(previous_start, tz='UTC+03:00')
previous_end_tz = pd.Timestamp(previous_end, tz='UTC+03:00')

# Convert POS timestamp to datetime with timezone awareness
...
pos_analysis = pos_df[
    (pos_df['timestamp'] >= analysis_start_tz) & 
    (pos_df['timestamp'] < analysis_end_tz) &
...
# Previous period
pos_previous = pos_df[
    (pos_df['timestamp'] >= previous_start_tz) & 
    (pos_df['timestamp'] < previous_end_tz) &
    (pos_df['is_refund'] == False)
].copy()
