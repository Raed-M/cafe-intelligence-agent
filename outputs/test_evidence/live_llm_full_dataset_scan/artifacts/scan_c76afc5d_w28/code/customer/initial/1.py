...
if len(reviews_analysis) > 0:
    ...
    finding_2 = {
        ...
        "claim": f"Reviews are distributed across {len(language_counts)} languages, with {language_counts.index[0]} being the most common ({language_counts.iloc[0]} reviews, {100*language_counts.iloc[0]/len(reviews_analysis):.1f}%).",
        "finding_type": "voice_of_customer",
        "metrics": {
...
if len(reviews_analysis) > 0:
    ...
    finding_3 = {
        ...
        "claim": f"Customer reviews originate from {len(platform_counts)} platforms, with {platform_counts.index[0]} accounting for {platform_counts.iloc[0]} reviews ({100*platform_counts.iloc[0]/len(reviews_analysis):.1f}% of total).",
        "finding_type": "voice_of_customer",
        "metrics": {
            "platform_distribution": {
                "value": dict(platform_counts),
                "unit": "count",
                "numerator": None,
                "denominator": None,
                "period_start": "2026-07-20T00:00:00+03:00",
