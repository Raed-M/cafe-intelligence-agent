...
if len(baseline_traffic_combined) > 0 and baseline_traffic_combined['door_count'].std() > 0:
    ...
    if anomalies_traffic:
        ...
        findings.append({
            ...
            'claim': f"Hourly door count on {top_traffic_anomaly['date']} at hour {top_traffic_anomaly['hour']} was {top_traffic_anomaly['door_count']} visitors, {abs(top_traffic_anomaly['z_score']):.2f} standard deviations from baseline mean of {top_traffic_anomaly['baseline_mean']:.1f}.",
            ...
            'metrics': {
                'hourly_door_count': {
                    ...
                    'numerator': None,
                    'denominator': None,
                    'period_start': str(top_traffic_anomaly['date']),
                    'period_end': str(top_traffic_anomaly['date'])
                },
            ...
            'coverage_notes': [
                f"Baseline computed from {len(baseline_traffic_combined)} hourly observations across 4 weeks",
