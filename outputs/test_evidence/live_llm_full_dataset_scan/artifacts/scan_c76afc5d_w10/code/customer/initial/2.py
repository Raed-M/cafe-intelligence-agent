...
if len(analysis_reviews) > 0:
    ...
    if len(lang_ratings) > 1:
        ...
        for lang in lang_ratings.keys():
            lang_claims.append(
                f"{lang}: {lang_ratings[lang]['count']} reviews "
                f"(avg {lang_ratings[lang]['avg_rating']:.2f} stars)"
            )
        
        finding_2 = {
            "title": "Sentiment Analysis by Language",
            "claim": f"Review sentiment varies by language. {', '.join(lang_claims)}.",
...
if len(analysis_reviews) > 0:
    ...
    non_empty_reviews = analysis_reviews[
        analysis_reviews['text'].notna() & 
        (analysis_reviews['text'].str.len() > 0)
    ]
    
    if len(non_empty_reviews) > 0:
        # Analyze sentiment indicators
        positive_text = ' '.join(
            positive_reviews[positive_reviews['text'].notna()]['text'].astype(str).tolist()
        ).lower()
        negative_text = ' '.join(
            negative_reviews[negative_reviews['text'].notna()]['text'].astype(str).tolist()
        ).lower()
        
        # Common positive indicators
        positive_keywords = [
            'good', 'great', 'excellent', 'love', 'best', 'amazing', 
            'perfect', 'delicious', 'tasty', 'nice'
        ]
        negative_keywords = [
            'bad', 'poor', 'terrible', 'hate', 'worst', 'awful', 
            'slow', 'cold', 'stale', 'rude'
        ]
        ...
        finding_3 = {
            ...
            "claim": (
                f"Analysis of {len(non_empty_reviews)} non-empty reviews reveals "
                f"{len(positive_reviews)} positive reviews (rating 4-5) and "
                f"{len(negative_reviews)} negative reviews (rating 1-2). "
                f"Positive reviews contain {positive_mentions} mentions of quality indicators, "
                f"while negative reviews contain {negative_mentions} mentions of quality concerns."
            ),
