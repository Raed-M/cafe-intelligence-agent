...
if len(reviews_analysis) > 0:
    ...
    finding1 = {
        ...
        "claim": f"During the analysis period ({analysis_start} to {analysis_end}), the average rating across {total_reviews} reviews is {avg_rating:.2f} out of 5, with distribution: 1-star: {rating_counts.get(1, 0)}, 2-star: {rating_counts.get(2, 0)}, 3-star: {rating_counts.get(3, 0)}, 4-star: {rating_counts.get(4, 0)}, 5-star: {rating_counts.get(5, 0)}.",
...
if len(reviews_analysis) > 0:
    ...
    finding2 = {
        ...
        "metrics": {
            ...
            "english_positive": {
                "value": english_sentiment.get('positive', 0),
                "unit": "count",
                "numerator": None,
                "denominator": None,
                "period_start": analysis_start,
            ...
            "arabic_positive": {
                "value": arabic_sentiment.get('positive', 0),
                "unit": "count",
                "numerator": None,
                "denominator": None,
                "period_start": analysis_start,
...
if len(topic_counts) > 0 and sum(topic_counts.values()) > 0:
    ...
    finding3 = {
        ...
        "claim": f"Among {len(reviews_analysis)} reviews with text content in the analysis period, the most frequently mentioned topics are: {top_topics[0][0]} ({top_topics[0][1]} mentions)" + (f", {top_topics[1][0]} ({top_topics[1][1]} mentions)" if len(top_topics) > 1 else "") + (f", {top_topics[2][0]} ({top_topics[2][1]} mentions)" if len(top_topics) > 2 else ""),
        "finding_type": "topic_frequency",
        "metrics": {
            "total_reviews_with_text": {
                "value": len(reviews_analysis[reviews_analysis['text'].notna()]),
                "unit": "count",
                "numerator": None,
                "denominator": None,
                "period_start": analysis_start,
        ...
        "source_names": source_names,
        "sample_size": len(reviews_analysis[reviews_analysis['text'].notna()]),
        "coverage_notes": [
    ...
    for topic, count in top_topics:
        result["findings"][2]["metrics"][f"{topic}_mentions"] = {
            "value": count,
            "unit": "count",
            "numerator": None,
            "denominator": None,
