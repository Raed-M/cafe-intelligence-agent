...
analysis_baskets = int(analysis_data['transaction_id'].nunique())
analysis_revenue = float(analysis_data['line_total_sar'].sum())
analysis_aov = float(analysis_revenue / analysis_baskets) if analysis_baskets > 0 else 0.0

# Calculate metrics for previous period
previous_baskets = int(previous_data['transaction_id'].nunique())
previous_revenue = float(previous_data['line_total_sar'].sum())
previous_aov = float(previous_revenue / previous_baskets) if previous_baskets > 0 else 0.0

# Calculate changes
basket_change = int(analysis_baskets - previous_baskets)
basket_pct_change = float((basket_change / previous_baskets * 100) if previous_baskets > 0 else 0.0)
revenue_change = float(analysis_revenue - previous_revenue)
revenue_pct_change = float((revenue_change / previous_revenue * 100) if previous_revenue > 0 else 0.0)
aov_change = float(analysis_aov - previous_aov)
aov_pct_change = float((aov_change / previous_aov * 100) if previous_aov > 0 else 0.0)
...
for cat in analysis_by_category.index:
    if cat in previous_by_category.index:
        curr_rev = float(analysis_by_category.loc[cat, 'revenue'])
        prev_rev = float(previous_by_category.loc[cat, 'revenue'])
        change = float(curr_rev - prev_rev)
        pct_change = float((change / prev_rev * 100) if prev_rev > 0 else 0.0)
        category_revenue_changes[cat] = {
            'current': curr_rev,
            'previous': prev_rev,
            'change': change,
            'pct_change': pct_change,
            'curr_baskets': int(analysis_by_category.loc[cat, 'baskets']),
            'prev_baskets': int(previous_by_category.loc[cat, 'baskets'])
        }
...
analysis_refund_count = int(len(analysis_refunds))
previous_refund_count = int(len(previous_refunds))
analysis_refund_value = float(analysis_refunds['line_total_sar'].sum())
previous_refund_value = float(previous_refunds['line_total_sar'].sum())
...
for sku in analysis_by_sku.index:
    if sku in previous_by_sku.index:
        curr_rev = float(analysis_by_sku.loc[sku, 'revenue'])
        prev_rev = float(previous_by_sku.loc[sku, 'revenue'])
        change = float(curr_rev - prev_rev)
        pct_change = float((change / prev_rev * 100) if prev_rev > 0 else 0.0)
        product_changes[sku] = {
            'name': str(analysis_by_sku.loc[sku, 'item_name_en']),
            'category': str(analysis_by_sku.loc[sku, 'category']),
            'current': curr_rev,
            'previous': prev_rev,
            'change': change,
            'pct_change': pct_change,
            'curr_baskets': int(analysis_by_sku.loc[sku, 'baskets']),
            'prev_baskets': int(previous_by_sku.loc[sku, 'baskets']),
            'curr_qty': float(analysis_by_sku.loc[sku, 'quantity']),
            'prev_qty': float(previous_by_sku.loc[sku, 'quantity'])
        }
...
aov_direction = "decreased" if aov_pct_change < 0 else "increased"
finding2 = {
    "title": f"Average Order Value {aov_direction.capitalize()} Week-over-Week",
    "claim": f"Average Order Value {aov_direction} from SAR {previous_aov:.2f} to SAR {analysis_aov:.2f}, representing a {aov_pct_change:.1f}% change. This indicates {'weaker' if aov_pct_change < 0 else 'stronger'} per-basket spending despite higher transaction volume.",
