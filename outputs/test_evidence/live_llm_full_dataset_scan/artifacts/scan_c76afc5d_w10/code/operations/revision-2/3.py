...
# Define periods using ISO format strings
...
# Convert to Asia/Riyadh timezone using UTC offset
riyadh_offset = pd.Timedelta(hours=3)
pos_df['timestamp'] = pos_df['timestamp'] + riyadh_offset
traffic_df['date'] = traffic_df['date'] + riyadh_offset
staff_df['date'] = staff_df['date'] + riyadh_offset
inventory_df['week_starting'] = inventory_df['week_starting'] + riyadh_offset
...
valid_transactions_analysis = int(pos_analysis[~pos_analysis['is_refund']]['transaction_id'].nunique())
dead_sensor_days_analysis = int(traffic_analysis[traffic_analysis['is_dead_sensor_day'] == True].shape[0])
valid_traffic_days_analysis = int(traffic_analysis[traffic_analysis['is_dead_sensor_day'] == False].shape[0])
total_footfall_analysis = int(traffic_analysis[traffic_analysis['is_dead_sensor_day'] == False]['door_count'].sum())
...
valid_transactions_previous = int(pos_previous[~pos_previous['is_refund']]['transaction_id'].nunique())
dead_sensor_days_previous = int(traffic_previous[traffic_previous['is_dead_sensor_day'] == True].shape[0])
valid_traffic_days_previous = int(traffic_previous[traffic_previous['is_dead_sensor_day'] == False].shape[0])
total_footfall_previous = int(traffic_previous[traffic_previous['is_dead_sensor_day'] == False]['door_count'].sum())
...
labour_cost_previous = float(staff_previous['labour_cost_sar'].sum())

# 4. Demand Analysis (total sales)
demand_analysis = float(pos_analysis[~pos_analysis['is_refund']]['line_total_sar'].sum())
demand_previous = float(pos_previous[~pos_previous['is_refund']]['line_total_sar'].sum())
...
# 6. Waste Analysis - get period-specific units_sold
units_sold_analysis = int(inventory_analysis['units_sold'].sum())
units_sold_previous = int(inventory_previous['units_sold'].sum())

known_waste_cost_analysis = float(inventory_analysis['known_waste_cost_sar'].sum())
known_waste_cost_previous = float(inventory_previous['known_waste_cost_sar'].sum())
...
if conversion_rate_previous > 0 and total_footfall_analysis > 0:
    findings.append({
        ...
        "coverage_notes": [
            f"Analysis period (2026-03-16 to 2026-03-23): {valid_traffic_days_analysis} valid traffic days, {dead_sensor_days_analysis} dead sensor days excluded",
            f"Previous period (2026-03-09 to 2026-03-16): {valid_traffic_days_previous} valid traffic days, {dead_sensor_days_previous} dead sensor days excluded",
...
if labour_to_demand_ratio_previous > 0 and demand_analysis > 0:
    findings.append({
        ...
        "coverage_notes": [
            f"Analysis period (2026-03-16 to 2026-03-23): {len(staff_analysis)} staff records",
            f"Previous period (2026-03-09 to 2026-03-16): {len(staff_previous)} staff records",
...
if waste_ratio_previous > 0 and units_sold_analysis > 0 and units_sold_previous > 0:
    findings.append({
        ...
        "coverage_notes": [
            f"Analysis period (2026-03-16 to 2026-03-23): {len(inventory_analysis)} inventory records, {units_sold_analysis} units sold",
            f"Previous period (2026-03-09 to 2026-03-16): {len(inventory_previous)} inventory records, {units_sold_previous} units sold",
        ...
        "assumptions": [
            ...
            "Period-specific units_sold denominators used for each period"
        ],
        "confidence": 0.75
    })
