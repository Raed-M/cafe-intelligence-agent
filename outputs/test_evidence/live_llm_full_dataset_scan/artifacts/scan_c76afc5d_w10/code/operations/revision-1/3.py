...
# Filter staff for analysis and previous periods - convert date column to datetime for comparison
staff_df['date'] = pd.to_datetime(staff_df['date'])
staff_analysis = staff_df[(staff_df['date'] >= pd.Timestamp(analysis_start.date(), tz=tz)) & (staff_df['date'] < pd.Timestamp(analysis_end.date(), tz=tz))].copy()
staff_previous = staff_df[(staff_df['date'] >= pd.Timestamp(previous_start.date(), tz=tz)) & (staff_df['date'] < pd.Timestamp(previous_end.date(), tz=tz))].copy()
