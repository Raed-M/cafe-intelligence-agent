...
# Remove timezone info from analysis period datetimes for comparison with tz-naive data
analysis_start_dt = analysis_start_dt.tz_localize(None)
analysis_end_dt = analysis_end_dt.tz_localize(None)
previous_start_dt = previous_start_dt.tz_localize(None)
previous_end_dt = previous_end_dt.tz_localize(None)
    ...
    for idx, row in price_changes_sorted.iterrows():
        ...
        findings.append({
            ...
            "coverage_notes": [
                ...
                f"Price change percentage calculated as (new_price - old_price) / old_price × 100 = ({new_price:.2f} - {old_price:.2f}) / {old_price:.2f} × 100 = {price_change_pct:.2f}%"
            ],
            "assumptions": [
