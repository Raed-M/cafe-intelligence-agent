...
# Convert review dates to datetime and ensure timezone awareness
...
if reviews_df['date'].dt.tz is None:
    ...
    # If it has timezone info but different from analysis period, convert
    reviews_df['date'] = reviews_df['date'].dt.tz_convert('UTC+03:00')

# Ensure analysis datetimes are also in UTC+3 for comparison
...
if len(reviews_analysis) > 0:
    ...
    finding2 = {
        ...
        "claim": f"During the analysis period, reviews were submitted in {len(language_dist)} language(s): {', '.join([f'{lang} ({count} reviews)' for lang, count in sorted(language_dist.items())])}.",
...
if len(reviews_analysis) > 0:
    ...
    positive_keywords_ar = ['جيد', 'رائع', 'ممتاز', 'عظيم', 'أحب', 'الأفضل', 'مثالي', 'رهيب', 'مذهل']
