...
if len(baseline_hourly_traffic) > 0 and len(traffic_analysis) > 0:
    ...
    if baseline_traffic_std > 0:
        ...
        anomalous_traffic = traffic_analysis[abs(traffic_analysis['z_score']) > 2.0].copy()
        anomalous_traffic['abs_z'] = abs(anomalous_traffic['z_score'])
        anomalous_traffic = anomalous_traffic.sort_values('abs_z', ascending=False)
