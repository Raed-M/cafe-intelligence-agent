...
analysis_aov = float(analysis_revenue / analysis_valid_txns) if analysis_valid_txns > 0 else 0.0
...
previous_aov = float(previous_revenue / previous_valid_txns) if previous_valid_txns > 0 else 0.0

# Calculate changes
revenue_change = float(analysis_revenue - previous_revenue)
revenue_change_pct = float((revenue_change / previous_revenue * 100)) if previous_revenue != 0 else 0.0
txn_change = int(analysis_valid_txns - previous_valid_txns)
txn_change_pct = float((txn_change / previous_valid_txns * 100)) if previous_valid_txns != 0 else 0.0
aov_change = float(analysis_aov - previous_aov)
aov_change_pct = float((aov_change / previous_aov * 100)) if previous_aov != 0 else 0.0
...
analysis_total_revenue = float(analysis_by_category['revenue'].sum())
...
previous_total_revenue = float(previous_by_category['revenue'].sum())
...
for cat in analysis_by_category['category'].unique():
    ...
    if len(analysis_cat) > 0 and len(previous_cat) > 0:
        analysis_pct = float(analysis_cat['pct_of_total'].values[0])
        previous_pct = float(previous_cat['pct_of_total'].values[0])
        pct_point_change = float(analysis_pct - previous_pct)
        
        analysis_rev = float(analysis_cat['revenue'].values[0])
        previous_rev = float(previous_cat['revenue'].values[0])
        rev_change = float(analysis_rev - previous_rev)
        rev_change_pct = float((rev_change / previous_rev * 100)) if previous_rev != 0 else 0.0
        ...
        if abs(pct_point_change) > 0.5:
            category_changes.append({
                ...
                'analysis_baskets': int(analysis_cat['baskets'].values[0]),
                'previous_baskets': int(previous_cat['baskets'].values[0])
            })
...
if len(analysis_by_sku) > 0:
    ...
    if len(prev_top) > 0:
        ...
        rev_change = float(top_sku['revenue'] - prev_top['revenue'])
        rev_change_pct = float((rev_change / prev_top['revenue'] * 100)) if prev_top['revenue'] != 0 else 0.0
        qty_change = int(top_sku['quantity'] - prev_top['quantity'])
        qty_change_pct = float((qty_change / prev_top['quantity'] * 100)) if prev_top['quantity'] != 0 else 0.0
        basket_change = int(top_sku['baskets'] - prev_top['baskets'])
        
        findings.append({
            "title": f"Top SKU Performance: {top_sku['item_name']}",
            "claim": f"The top-revenue SKU {top_sku['item_name']} (SKU: {top_sku_code}) generated SAR {top_sku['revenue']:.2f} across {int(top_sku['quantity'])} units and {int(top_sku['baskets'])} baskets in the analysis week (2026-03-23 to 2026-03-30), compared to SAR {prev_top['revenue']:.2f} ({int(prev_top['quantity'])} units, {int(prev_top['baskets'])} baskets) in the prior week. Revenue changed by SAR {rev_change:.2f} ({rev_change_pct:.2f}%), quantity by {qty_change} units ({qty_change_pct:.2f}%), and basket penetration by {basket_change} baskets.",
            "finding_type": "top_sku_performance",
            "metrics": {
                "analysis_sku_revenue_sar": {
                    "value": round(float(top_sku['revenue']), 2),
                    "unit": "SAR",
                    "numerator": None,
                    "denominator": None,
                    "period_start": "2026-03-23T00:00:00+03:00",
                ...
                "previous_sku_revenue_sar": {
                    "value": round(float(prev_top['revenue']), 2),
                    "unit": "SAR",
                    "numerator": None,
                    "denominator": None,
                    "period_start": "2026-03-16T00:00:00+03:00",
                ...
                "sku_revenue_change_sar": {
                    ...
                    "unit": "SAR",
                    "numerator": round(float(top_sku['revenue']), 2),
                    "denominator": round(float(prev_top['revenue']), 2),
                    "period_start": "2026-03-23T00:00:00+03:00",
                    "period_end": "2026-03-30T00:00:00+03:00"
                },
