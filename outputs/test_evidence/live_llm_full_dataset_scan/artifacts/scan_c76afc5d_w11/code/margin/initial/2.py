...
if len(inv_analysis) > 0:
    inv_analysis_copy = inv_analysis.copy()
    inv_analysis_copy['sku'] = inv_analysis_copy['sku'].astype(str)
    
    # Use unit_cost_sar directly from inventory table (it already has this column)
    waste_data = inv_analysis_copy[inv_analysis_copy['units_wasted'].notna() & (inv_analysis_copy['units_wasted'] > 0)].copy()
    
    if len(waste_data) > 0:
        # Calculate waste cost using unit_cost_sar from inventory
        ...
        findings.append({
            ...
            "assumptions": [
                "Unit costs in inventory are accurate and applicable",
