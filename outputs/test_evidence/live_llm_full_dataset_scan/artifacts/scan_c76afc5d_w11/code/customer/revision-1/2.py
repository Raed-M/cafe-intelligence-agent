...
if len(reviews_analysis) > 0:
    ...
    finding_1 = {
        ...
        "claim": f"During the analysis period ({analysis_start.date()} to {analysis_end.date()}), the average rating across {len(reviews_analysis)} reviews is {avg_rating:.2f} stars, with distribution: {dict((int(k), int(v)) for k, v in rating_counts.items())}.",
    ...
    finding_3 = {
        ...
        "metrics": {
            ...
            "high_rating_percentage": {
                "value": float(round(100 * high_rating_reviews / len(reviews_analysis), 1)) if len(reviews_analysis) > 0 else 0.0,
                "unit": "percent",
                "numerator": int(high_rating_reviews),
                "denominator": int(len(reviews_analysis)),
                "period_start": analysis_start.isoformat(),
                "period_end": analysis_end.isoformat()
            },
            "low_rating_percentage": {
                "value": float(round(100 * low_rating_reviews / len(reviews_analysis), 1)) if len(reviews_analysis) > 0 else 0.0,
                "unit": "percent",
                "numerator": int(low_rating_reviews),
                "denominator": int(len(reviews_analysis)),
                "period_start": analysis_start.isoformat(),
                "period_end": analysis_end.isoformat()
            }
