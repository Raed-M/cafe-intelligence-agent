...
# Convert to the analysis timezone
pos_df['timestamp'] = pos_df['timestamp'].dt.tz_convert('UTC+03:00')

# Ensure analysis period datetimes are timezone-aware
...
if previous_start.tzinfo is None:
    previous_start = previous_start.replace(tzinfo=timezone.utc)
...
# Convert analysis period to UTC for comparison with menu dates
...
recently_launched = menu_df[
    (menu_df['launch_date'] >= (analysis_start_utc - timedelta(days=7))) & 
    (menu_df['launch_date'] <= analysis_end_utc)
]
