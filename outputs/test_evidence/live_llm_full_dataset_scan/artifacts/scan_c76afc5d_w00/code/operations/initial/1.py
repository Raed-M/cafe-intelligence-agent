...
analysis_start = tz.localize(datetime(2026, 1, 5, 0, 0, 0))
analysis_end = tz.localize(datetime(2026, 1, 12, 0, 0, 0))
prev_start = tz.localize(datetime(2025, 12, 29, 0, 0, 0))
prev_end = tz.localize(datetime(2026, 1, 5, 0, 0, 0))
...
if total_footfall > 0:
    ...
    findings.append({
        ...
        "coverage_notes": [
            ...
            f"Excluded {len(traffic_df[(traffic_df['date'] >= analysis_start.date()) & (traffic_df['date'] < analysis_end.date()) & (traffic_df['is_dead_sensor_day'] == True)])} dead sensor days",
...
inventory_analysis = inventory_df[
    (inventory_df['week_starting'] >= analysis_start.date()) &
    (inventory_df['week_starting'] < analysis_end.date())
].copy()
