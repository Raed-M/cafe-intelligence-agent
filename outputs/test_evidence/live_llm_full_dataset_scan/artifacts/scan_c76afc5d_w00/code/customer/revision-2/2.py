...
# Convert to naive UTC for comparison with naive datetime bounds
analysis_start_dt_utc = analysis_start_dt.tz_convert('UTC').tz_localize(None)
analysis_end_dt_utc = analysis_end_dt.tz_convert('UTC').tz_localize(None)
reviews_df['date_naive'] = reviews_df['date'].dt.tz_localize(None)

# Filter reviews for analysis period
reviews_analysis = reviews_df[
    (reviews_df['date_naive'] >= analysis_start_dt_utc) & 
    (reviews_df['date_naive'] < analysis_end_dt_utc)
].copy()
...
if len(reviews_analysis) > 0:
    ...
    finding2 = {
        ...
        "coverage_notes": [
            ...
            f"English reviews: {len(en_reviews)} ({round(len(en_reviews)/len(reviews_analysis)*100, 1)}%)" if len(reviews_analysis) > 0 else "English reviews: 0",
            f"Arabic reviews: {len(ar_reviews)} ({round(len(ar_reviews)/len(reviews_analysis)*100, 1)}%)" if len(reviews_analysis) > 0 else "Arabic reviews: 0",
