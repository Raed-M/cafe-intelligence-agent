...
# Convert timestamps to datetime and make timezone-naive
pos_df['timestamp'] = pd.to_datetime(pos_df['timestamp']).dt.tz_localize(None)
pos_df['calendar_date'] = pd.to_datetime(pos_df['calendar_date']).dt.tz_localize(None)
traffic_df['date'] = pd.to_datetime(traffic_df['date']).dt.tz_localize(None)
traffic_df['hour'] = pd.to_datetime(traffic_df['hour']).dt.tz_localize(None)
inventory_df['week_starting'] = pd.to_datetime(inventory_df['week_starting']).dt.tz_localize(None)
reviews_df['date'] = pd.to_datetime(reviews_df['date']).dt.tz_localize(None)

# Define analysis periods (timezone-naive)
analysis_start = datetime(2026, 1, 5, 0, 0, 0)
analysis_end = datetime(2026, 1, 12, 0, 0, 0)
baseline_start = datetime(2025, 12, 29, 0, 0, 0)
