            ...
            if abs(z_score) > 2.0:  # 2 standard deviations
                findings.append({
                    ...
                    'coverage_notes': [
                        ...
                        f"Baseline: 4 weeks from {baseline_periods[0][0].date()} to {baseline_periods[-1][1].date()}",
        ...
        for idx, row in traffic_analysis.iterrows():
            ...
            if abs(z_score) > 2.5:  # Higher threshold for hourly data
                findings.append({
                    ...
                    'claim': f"Door count on {row['date']} at hour {row['hour']} was {row['door_count']} visitors, which is {abs(z_score):.2f} standard deviations from the baseline mean of {baseline_traffic_mean:.1f}.",
                    ...
                    'metrics': {
                        'observed_hourly_door_count': {
                            ...
                            'numerator': None,
                            'denominator': None,
                            'period_start': f"{row['date']}T{int(row['hour']):02d}:00:00+03:00",
                            'period_end': f"{row['date']}T{int(row['hour']):02d}:59:59+03:00"
                        },
                ...
                findings.append({
                    ...
                    'metrics': {
                        'observed_weekly_waste_cost': {
                            ...
                            'numerator': None,
                            'denominator': None,
                            'period_start': str(week.date()),
                            'period_end': str((week + timedelta(days=6)).date())
                        },
