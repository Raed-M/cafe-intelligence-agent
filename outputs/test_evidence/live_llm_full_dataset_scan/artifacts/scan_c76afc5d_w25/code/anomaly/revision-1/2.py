        ...
        for idx, row in traffic_analysis.iterrows():
            ...
            if abs(z_score) > 2.5:  # Higher threshold for hourly data
                ...
                findings.append({
                    ...
                    'claim': f"Door count on {row['date'].date()} at hour {hour_str} was {row['door_count']} visitors, which is {abs(z_score):.2f} standard deviations from the baseline mean of {baseline_traffic_mean:.1f}.",
                    ...
                    'metrics': {
                        'observed_hourly_door_count': {
                            ...
                            'numerator': None,
                            'denominator': None,
                            'period_start': f"{row['date'].date()}T{hour_str}:00:00+03:00",
                            'period_end': f"{row['date'].date()}T{hour_str}:59:59+03:00"
                        },
