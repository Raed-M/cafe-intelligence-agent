...
analysis_txns = int(analysis_clean['transaction_id'].nunique())
previous_txns = int(previous_clean['transaction_id'].nunique())
analysis_revenue = float(analysis_clean['line_total_sar'].sum())
previous_revenue = float(previous_clean['line_total_sar'].sum())
...
if top_category_previous is not None:
    cat_revenue_change = float(top_category_analysis['revenue']) - float(top_category_previous['revenue'])
    cat_revenue_pct_change = (cat_revenue_change / float(top_category_previous['revenue']) * 100) if float(top_category_previous['revenue']) > 0 else 0
    cat_share_change = float(top_category_analysis['revenue_share']) - float(top_category_previous['revenue_share'])
    
    findings.append({
        "title": f"Top Category Revenue and Share: {top_category_analysis['category']}",
        "claim": f"The {top_category_analysis['category']} category generated SAR {float(top_category_analysis['revenue']):.2f} in the analysis period ({float(top_category_analysis['revenue_share']):.2f}% of total revenue) compared to SAR {float(top_category_previous['revenue']):.2f} in the previous period ({float(top_category_previous['revenue_share']):.2f}% of total revenue), representing a {cat_revenue_pct_change:.2f}% change in category revenue and a {cat_share_change:.2f} percentage point change in revenue share.",
        "finding_type": "category_mix_and_revenue",
        "metrics": {
            "category_revenue_analysis": {
                "value": round(float(top_category_analysis['revenue']), 2),
                "unit": "SAR",
                "numerator": None,
                "denominator": None,
                "period_start": "2026-04-27T00:00:00+03:00",
            ...
            "category_revenue_previous": {
                "value": round(float(top_category_previous['revenue']), 2),
                "unit": "SAR",
                "numerator": None,
                "denominator": None,
                "period_start": "2026-04-20T00:00:00+03:00",
            ...
            "category_revenue_pct_change": {
                ...
                "denominator": round(float(top_category_previous['revenue']), 2),
                "period_start": "2026-04-20T00:00:00+03:00",
                "period_end": "2026-05-04T00:00:00+03:00"
            },
            "category_share_analysis": {
                "value": round(float(top_category_analysis['revenue_share']), 2),
                "unit": "percent",
                "numerator": round(float(top_category_analysis['revenue']), 2),
                "denominator": round(analysis_clean['line_total_sar'].sum(), 2),
                "period_start": "2026-04-27T00:00:00+03:00",
                "period_end": "2026-05-04T00:00:00+03:00"
            },
            "category_share_previous": {
                "value": round(float(top_category_previous['revenue_share']), 2),
                "unit": "percent",
                "numerator": round(float(top_category_previous['revenue']), 2),
...
if top_channel_previous is not None:
    ch_revenue_change = float(top_channel_analysis['revenue']) - float(top_channel_previous['revenue'])
    ch_revenue_pct_change = (ch_revenue_change / float(top_channel_previous['revenue']) * 100) if float(top_channel_previous['revenue']) > 0 else 0
    ch_txn_change = int(top_channel_analysis['txn_count']) - int(top_channel_previous['txn_count'])
    ch_txn_pct_change = (ch_txn_change / int(top_channel_previous['txn_count']) * 100) if int(top_channel_previous['txn_count']) > 0 else 0
    ch_aov_change = float(top_channel_analysis['avg_order_value']) - float(top_channel_previous['avg_order_value'])
    ch_aov_pct_change = (ch_aov_change / float(top_channel_previous['avg_order_value']) * 100) if float(top_channel_previous['avg_order_value']) > 0 else 0
    
    findings.append({
        "title": f"Top Channel Performance: {top_channel_analysis['channel']}",
        "claim": f"The {top_channel_analysis['channel']} channel generated SAR {float(top_channel_analysis['revenue']):.2f} from {int(top_channel_analysis['txn_count'])} transactions in the analysis period, compared to SAR {float(top_channel_previous['revenue']):.2f} from {int(top_channel_previous['txn_count'])} transactions in the previous period. Revenue changed by {ch_revenue_pct_change:.2f}%, transaction count changed by {ch_txn_pct_change:.2f}%, and average order value changed from SAR {float(top_channel_previous['avg_order_value']):.2f} to SAR {float(top_channel_analysis['avg_order_value']):.2f} ({ch_aov_pct_change:.2f}% change).",
        "finding_type": "channel_mix_and_performance",
        "metrics": {
            "channel_revenue_analysis": {
                "value": round(float(top_channel_analysis['revenue']), 2),
                "unit": "SAR",
                "numerator": None,
                "denominator": None,
                "period_start": "2026-04-27T00:00:00+03:00",
            ...
            "channel_revenue_previous": {
                "value": round(float(top_channel_previous['revenue']), 2),
                "unit": "SAR",
                "numerator": None,
                "denominator": None,
                "period_start": "2026-04-20T00:00:00+03:00",
            ...
            "channel_revenue_pct_change": {
                ...
                "denominator": round(float(top_channel_previous['revenue']), 2),
                "period_start": "2026-04-20T00:00:00+03:00",
                "period_end": "2026-05-04T00:00:00+03:00"
            },
            ...
            "channel_aov_analysis": {
                "value": round(float(top_channel_analysis['avg_order_value']), 2),
                "unit": "SAR",
                "numerator": None,
                "denominator": None,
                "period_start": "2026-04-27T00:00:00+03:00",
            ...
            "channel_aov_previous": {
                "value": round(float(top_channel_previous['avg_order_value']), 2),
                "unit": "SAR",
                "numerator": None,
                "denominator": None,
                "period_start": "2026-04-20T00:00:00+03:00",
            ...
            "channel_aov_pct_change": {
                ...
                "denominator": round(float(top_channel_previous['avg_order_value']), 2),
                "period_start": "2026-04-20T00:00:00+03:00",
                "period_end": "2026-05-04T00:00:00+03:00"
            }
        },
