...
# Convert analysis_start and analysis_end to date objects for comparison
analysis_start_date = analysis_start.date()
analysis_end_date = analysis_end.date()
baseline_start_date = baseline_periods[0][0].date()
baseline_end_date = baseline_periods[0][1].date()
...
traffic_baseline = traffic_df[
    (traffic_df['date'].dt.date >= baseline_start_date) & 
    (traffic_df['date'].dt.date < baseline_end_date) &
    (traffic_df['is_dead_sensor_day'] == False)
].copy()
