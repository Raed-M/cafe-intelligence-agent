...
if len(reviews_analysis) > 0:
    ...
    finding1 = {
        ...
        "claim": f"During the analysis period (2026-04-27 to 2026-05-04), the average rating across {len(reviews_analysis)} reviews was {avg_rating:.2f} out of 5, with {language_counts.get('en', 0)} English and {language_counts.get('ar', 0)} Arabic reviews.",
...
if len(reviews_analysis) > 0:
    ...
    negative_count = len(reviews_analysis[reviews_analysis['rating'] <= 2])
    ...
    if mentioned_topics:
        finding2 = {
            ...
            "claim": f"Among {len(reviews_analysis)} reviews in the analysis period, {positive_count} were positive (rating ≥4), {neutral_count} neutral (rating=3), and {negative_count} negative (rating ≤2). Key topics mentioned include: {', '.join([f'{k} ({v} mentions)' for k, v in sorted(mentioned_topics.items(), key=lambda x: x[1], reverse=True)[:3]])}.",
            ...
            "coverage_notes": [
                ...
                f"Topics identified through keyword matching in {len(english_reviews)} English and {len(arabic_reviews)} Arabic reviews",
...
if len(reviews_analysis) > 0:
    ...
    if len(high_rated) > 0 and len(low_rated) > 0:
        ...
        finding3 = {
            ...
            "claim": f"During the analysis period, {len(high_rated)} reviews were highly rated (≥4 stars) compared to {len(low_rated)} low-rated reviews (≤2 stars), indicating a {round(len(high_rated)/len(reviews_analysis)*100, 1)}% positive sentiment rate.",
            "finding_type": "sentiment_comparison",
            "metrics": {
                "high_rated_count": {
                    "value": len(high_rated),
                    "unit": "count",
                    "numerator": len(high_rated),
                    "denominator": len(reviews_analysis),
                    "period_start": analysis_start,
                    "period_end": analysis_end
                },
                "low_rated_count": {
                    "value": len(low_rated),
                    "unit": "count",
                    "numerator": len(low_rated),
                    "denominator": len(reviews_analysis),
                    "period_start": analysis_start,
                    "period_end": analysis_end
                },
                "positive_sentiment_rate": {
                    "value": round(len(high_rated)/len(reviews_analysis)*100, 1),
                    "unit": "percent",
                    "numerator": len(high_rated),
                    "denominator": len(reviews_analysis),
                    "period_start": analysis_start,
