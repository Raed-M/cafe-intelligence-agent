...
# Timestamps are already in the correct timezone from the context
...
# Convert traffic dates to datetime64 for consistent comparison
traffic_df['date'] = pd.to_datetime(traffic_df['date'])
...
traffic_previous = traffic_df[(traffic_df['date'] >= pd.Timestamp(previous_start.date())) & (traffic_df['date'] < pd.Timestamp(previous_end.date()))].copy()

staff_df['date'] = pd.to_datetime(staff_df['date'])
staff_analysis = staff_df[(staff_df['date'] >= pd.Timestamp(analysis_start.date())) & (staff_df['date'] < pd.Timestamp(analysis_end.date()))].copy()
staff_previous = staff_df[(staff_df['date'] >= pd.Timestamp(previous_start.date())) & (staff_df['date'] < pd.Timestamp(previous_end.date()))].copy()
