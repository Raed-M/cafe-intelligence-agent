...
# Define periods (using UTC offset +03:00 for Asia/Riyadh)
# Create timezone-aware datetimes using UTC and then represent with +03:00 offset
analysis_start = pd.Timestamp('2026-06-15T00:00:00', tz='UTC').tz_convert('UTC').replace(tzinfo=None)
analysis_start = pd.Timestamp('2026-06-15T00:00:00+03:00', tz='UTC')
analysis_end = pd.Timestamp('2026-06-22T00:00:00+03:00', tz='UTC')
prev_start = pd.Timestamp('2026-06-08T00:00:00+03:00', tz='UTC')
prev_end = pd.Timestamp('2026-06-15T00:00:00+03:00', tz='UTC')

# Convert timestamps to UTC for comparison
pos_df['timestamp'] = pd.to_datetime(pos_df['timestamp'], utc=True)
traffic_df['date'] = pd.to_datetime(traffic_df['date']).dt.tz_localize('UTC')
staff_df['date'] = pd.to_datetime(staff_df['date']).dt.tz_localize('UTC')
...
inventory_df['week_starting'] = pd.to_datetime(inventory_df['week_starting']).dt.tz_localize('UTC')
...
staff_analysis = staff_df[staff_df['date'] >= analysis_start.normalize()]
staff_analysis = staff_analysis[staff_analysis['date'] < analysis_end.normalize()]
...
# Previous period
staff_prev = staff_df[staff_df['date'] >= prev_start.normalize()]
staff_prev = staff_prev[staff_df['date'] < prev_end.normalize()]
