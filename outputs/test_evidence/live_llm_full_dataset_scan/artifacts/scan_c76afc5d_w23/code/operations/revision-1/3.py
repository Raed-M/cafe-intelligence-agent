...
valid_sales_analysis = int(pos_analysis[pos_analysis['is_refund'] == False]['transaction_id'].nunique())
valid_footfall_analysis = int(traffic_analysis[traffic_analysis['is_dead_sensor_day'] == False]['door_count'].sum())
...
valid_sales_previous = int(pos_previous[pos_previous['is_refund'] == False]['transaction_id'].nunique())
valid_footfall_previous = int(traffic_previous[traffic_previous['is_dead_sensor_day'] == False]['door_count'].sum())
...
# Analysis period
total_labour_cost_analysis = float(staff_analysis['labour_cost_sar'].sum())
total_revenue_analysis = float(pos_analysis[pos_analysis['is_refund'] == False]['line_total_sar'].sum())
labour_cost_ratio_analysis = total_labour_cost_analysis / total_revenue_analysis if total_revenue_analysis > 0 else None

# Previous period
total_labour_cost_previous = float(staff_previous['labour_cost_sar'].sum())
total_revenue_previous = float(pos_previous[pos_previous['is_refund'] == False]['line_total_sar'].sum())
...
if labour_cost_ratio_analysis is not None and labour_cost_ratio_previous is not None:
    findings.append({
        ...
        'sample_size': int(len(staff_analysis) + len(staff_previous)),
        'coverage_notes': [
...
if len(inventory_analysis) > 0 and len(inventory_previous) > 0:
    ...
    if waste_ratio_analysis is not None and waste_ratio_previous is not None:
        findings.append({
            ...
            'sample_size': int(len(inventory_analysis) + len(inventory_previous)),
            'coverage_notes': [
...
with open(output_path, 'w') as f:
    json.dump(output, f, indent=2, default=to_json_serializable)
