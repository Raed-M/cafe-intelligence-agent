...
if conversion_analysis is not None and conversion_previous is not None:
    ...
    if abs(conversion_change_pct) > 5:  # Only report if meaningful change
        findings.append({
            ...
            "sample_size": int(total_footfall_analysis),
            "coverage_notes": [
                f"Analysis period: {dead_sensor_days_analysis} dead sensor days excluded from {len(traffic_analysis)} total traffic records",
                f"Previous period: {dead_sensor_days_previous} dead sensor days excluded from {len(traffic_previous)} total traffic records",
...
if labour_cost_pct_analysis is not None and labour_cost_pct_previous is not None:
    ...
    findings.append({
        "title": "Labour Cost Ratio Change Week-over-Week",
        "claim": f"Labour cost as percentage of revenue decreased from {labour_cost_pct_previous:.1f}% to {labour_cost_pct_analysis:.1f}%, a change of {labour_cost_pct_change:.1f} percentage points.",
        ...
        "sample_size": int(staff_analysis.shape[0]),
        "coverage_notes": [
...
if waste_rate_analysis is not None and waste_rate_previous is not None:
    ...
    findings.append({
        "title": "Known Product Waste Rate Change",
        "claim": f"Known waste rate decreased from {waste_rate_previous*100:.1f}% to {waste_rate_analysis*100:.1f}%, with known waste cost of {waste_cost_analysis:.2f} SAR in analysis period.",
        "finding_type": "waste_metric",
        "metrics": {
            "waste_rate_previous": {
                "value": round(waste_rate_previous, 4),
                "unit": "ratio",
                "numerator": units_wasted_previous,
                "denominator": units_sold_previous + units_wasted_previous,
                "period_start": previous_start.isoformat(),
                "period_end": previous_end.isoformat()
            },
            "waste_rate_analysis": {
                ...
                "unit": "ratio",
                "numerator": units_wasted_analysis,
                "denominator": units_sold_analysis + units_wasted_analysis,
                "period_start": analysis_start.isoformat(),
                "period_end": analysis_end.isoformat()
            },
            "waste_cost_analysis": {
