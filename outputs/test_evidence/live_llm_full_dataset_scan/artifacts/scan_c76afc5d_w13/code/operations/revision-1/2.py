...
if labour_cost_pct_analysis is not None and labour_cost_pct_previous is not None:
    ...
    if labour_cost_pct_change < 0:
        direction = "decreased"
        efficiency_note = "indicating improved labour cost efficiency"
    else:
        direction = "increased"
        efficiency_note = "indicating reduced labour cost efficiency"
    
    findings.append({
        "title": "Labour Cost Ratio Change Week-over-Week",
        "claim": f"Labour cost as percentage of revenue {direction} from {labour_cost_pct_previous:.1f}% to {labour_cost_pct_analysis:.1f}%, a change of {labour_cost_pct_change:.1f} percentage points, {efficiency_note}.",
