...
# Define periods using pandas timezone-aware datetime (UTC+3 offset)
utc_plus_3 = pd.DatetimeIndex(['2026-01-01'], tz='UTC').tz_convert('Asia/Riyadh').tz
analysis_start = pd.Timestamp('2026-04-06 00:00:00', tz=utc_plus_3)
analysis_end = pd.Timestamp('2026-04-13 00:00:00', tz=utc_plus_3)
previous_start = pd.Timestamp('2026-03-30 00:00:00', tz=utc_plus_3)
previous_end = pd.Timestamp('2026-04-06 00:00:00', tz=utc_plus_3)

# Convert timestamps to timezone-aware
pos_df['timestamp'] = pd.to_datetime(pos_df['timestamp'], utc=True).dt.tz_convert(utc_plus_3)
traffic_df['date'] = pd.to_datetime(traffic_df['date']).dt.tz_localize(utc_plus_3)
staff_df['shift_start'] = pd.to_datetime(staff_df['shift_start'], utc=True).dt.tz_convert(utc_plus_3)
staff_df['shift_end'] = pd.to_datetime(staff_df['shift_end'], utc=True).dt.tz_convert(utc_plus_3)
inventory_df['week_starting'] = pd.to_datetime(inventory_df['week_starting']).dt.tz_localize(utc_plus_3)
...
if analysis_labour['ratio'] is not None and previous_labour['ratio'] is not None:
    ...
    if labour_ratio_pct_change is not None:
        ...
        finding_2 = {
            ...
            "claim": f"Labour cost as % of revenue {direction} from {previous_labour['ratio']:.4f} ({previous_labour['total_labour_cost']:.2f} SAR / {previous_labour['net_revenue']:.2f} SAR revenue) in the previous period to {analysis_labour['ratio']:.4f} ({analysis_labour['total_labour_cost']:.2f} SAR / {analysis_labour['net_revenue']:.2f} SAR revenue) in the analysis period, a {abs(labour_ratio_pct_change):.1f}% {direction} change.",
            "finding_type": "operational_metric",
            "metrics": {
