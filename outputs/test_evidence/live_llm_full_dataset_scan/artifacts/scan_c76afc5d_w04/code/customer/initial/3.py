...
# Convert to datetime for filtering - use utc=True to handle timezone-aware strings
...
if len(analysis_reviews) > 0:
    ...
    finding2 = {
        ...
        "claim": f"Of {len(analysis_reviews)} reviews, {int(language_counts.get('en', 0))} are in English and {int(language_counts.get('ar', 0))} are in Arabic. Common topics mentioned include: {', '.join([f'{topic} ({count} reviews)' for topic, count in sorted(topics_found.items(), key=lambda x: x[1], reverse=True)[:3]])}.",
        ...
        "sample_size": int(len(analysis_reviews)),
        "coverage_notes": [
            f"Language coverage: English {int(language_counts.get('en', 0))}/{len(analysis_reviews)}, Arabic {int(language_counts.get('ar', 0))}/{len(analysis_reviews)}",
...
if len(analysis_reviews) > 0:
    ...
    if len(language_ratings) > 1 and 'en' in language_ratings.index and 'ar' in language_ratings.index:
        finding3 = {
            "title": "Rating Differences by Review Language",
            "claim": f"English reviews (n={int(language_ratings.loc['en', 'count'])}) averaged {float(language_ratings.loc['en', 'mean']):.2f} stars, while Arabic reviews (n={int(language_ratings.loc['ar', 'count'])}) averaged {float(language_ratings.loc['ar', 'mean']):.2f} stars.",
