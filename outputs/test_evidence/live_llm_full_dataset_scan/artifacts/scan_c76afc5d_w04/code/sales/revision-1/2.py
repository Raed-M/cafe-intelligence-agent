...
analysis_txns = int(analysis_data[~analysis_data['is_refund']]['transaction_id'].nunique())
previous_txns = int(previous_data[~previous_data['is_refund']]['transaction_id'].nunique())
...
txn_change = int(analysis_txns - previous_txns)
...
for cat in analysis_cat_revenue.index:
    if cat in previous_cat_revenue.index:
        curr = float(analysis_cat_revenue[cat])
        prev = float(previous_cat_revenue[cat])
...
if category_changes:
    ...
    if abs(cat_metrics['pct_change']) > 3:
        findings.append({
            ...
            "claim": f"Category '{cat_name}' generated {cat_metrics['current']:.2f} SAR (net of refunds) in analysis week vs {cat_metrics['previous']:.2f} SAR (net of refunds) in previous week, representing a {cat_metrics['pct_change']:.1f}% change ({cat_metrics['change']:.2f} SAR). This category showed notable performance variance between the two periods.",
            ...
            "sample_size": int(len(analysis_with_cat[analysis_with_cat['category'] == cat_name])),
            "coverage_notes": [
...
for ch in analysis_by_channel.index:
    if ch in previous_by_channel.index:
        curr_rev = float(analysis_by_channel.loc[ch, 'revenue'])
        prev_rev = float(previous_by_channel.loc[ch, 'revenue'])
        curr_txn = int(analysis_by_channel.loc[ch, 'transactions'])
        prev_txn = int(previous_by_channel.loc[ch, 'transactions'])
...
if channel_changes:
    ...
    if abs(ch_metrics['rev_pct_change']) > 2:
        findings.append({
            ...
            "claim": f"Channel '{ch_name}' generated {ch_metrics['curr_rev']:.2f} SAR (net of refunds) across {ch_metrics['curr_txn']} transactions in analysis week, compared to {ch_metrics['prev_rev']:.2f} SAR (net of refunds) across {ch_metrics['prev_txn']} transactions in previous week. Revenue changed {ch_metrics['rev_pct_change']:.1f}% ({ch_metrics['rev_change']:.2f} SAR), while transaction count changed {ch_metrics['txn_pct_change']:.1f}% ({int(ch_metrics['txn_change']):+d} txns).",
            ...
            "metrics": {
                ...
                "channel_transaction_change": {
                    "value": int(ch_metrics['txn_change']),
                    "unit": "count",
                    "numerator": None,
                    "denominator": None,
                    "period_start": "2026-02-02T00:00:00+03:00",
                ...
                "channel_transaction_pct_change": {
                    ...
                    "unit": "%",
                    "numerator": int(ch_metrics['txn_change']),
            ...
            "source_names": ["pos"],
            "sample_size": int(len(analysis_data[analysis_data['channel'] == ch_name])),
            "coverage_notes": [
                "Revenue includes refunds (net calculation per metric definition)",
