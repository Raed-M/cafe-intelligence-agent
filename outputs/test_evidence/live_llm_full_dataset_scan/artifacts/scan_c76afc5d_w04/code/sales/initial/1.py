...
trailing_txns = trailing_data['transaction_id'].nunique()
...
for cat in analysis_category_revenue.index:
    analysis_rev = analysis_category_revenue.get(cat, 0)
    previous_rev = previous_category_revenue.get(cat, 0)
    if previous_rev != 0:
...
for ch in analysis_channel_revenue.index:
    analysis_rev = analysis_channel_revenue.get(ch, 0)
    previous_rev = previous_channel_revenue.get(ch, 0)
    analysis_txn = analysis_channel_txns.get(ch, 0)
    previous_txn = previous_channel_txns.get(ch, 0)
