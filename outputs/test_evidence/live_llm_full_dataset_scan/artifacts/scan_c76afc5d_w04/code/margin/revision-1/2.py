...
# Convert timestamp columns to datetime with timezone-naive handling
...
emails_df['date'] = pd.to_datetime(emails_df['date'], utc=False)
emails_df['effective_date'] = pd.to_datetime(emails_df['effective_date'], utc=False)

# Convert analysis period to timezone-naive for comparison with tz-naive data
...
if not price_changes.empty:
    ...
    if is_future:
        claim_text += f" Note: effective date is {(effective_date - analysis_end_naive).days} days after analysis period end."
    ...
    finding_3 = {
        ...
        "coverage_notes": [
            f"Analysis period: {analysis_start.isoformat()} to {analysis_end.isoformat()}",
            f"Email extraction date: {latest_change['date'].isoformat() if pd.notna(latest_change['date']) else 'unknown'}",
        ...
        "confidence": 0.85 if is_future else 0.90
    }
