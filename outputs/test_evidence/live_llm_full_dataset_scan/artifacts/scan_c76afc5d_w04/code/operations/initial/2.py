...
# Define periods using pandas Timestamp with UTC offset
...
traffic_df['date'] = pd.to_datetime(traffic_df['date'], utc=False)
traffic_df['date'] = traffic_df['date'].dt.tz_localize('UTC').dt.tz_convert('Asia/Riyadh')
staff_df['date'] = pd.to_datetime(staff_df['date'], utc=False)
staff_df['date'] = staff_df['date'].dt.tz_localize('UTC').dt.tz_convert('Asia/Riyadh')
staff_df['shift_start'] = pd.to_datetime(staff_df['shift_start'], utc=False)
staff_df['shift_start'] = staff_df['shift_start'].dt.tz_localize('UTC').dt.tz_convert('Asia/Riyadh')
staff_df['shift_end'] = pd.to_datetime(staff_df['shift_end'], utc=False)
staff_df['shift_end'] = staff_df['shift_end'].dt.tz_localize('UTC').dt.tz_convert('Asia/Riyadh')
inventory_df['week_starting'] = pd.to_datetime(inventory_df['week_starting'], utc=False)
...
traffic_analysis = traffic_df[
    (traffic_df['date'].dt.normalize() >= analysis_start.normalize()) & 
    (traffic_df['date'].dt.normalize() < analysis_end.normalize()) &
    (traffic_df['is_dead_sensor_day'] == False)
].copy()

traffic_previous = traffic_df[
    (traffic_df['date'].dt.normalize() >= previous_start.normalize()) & 
    (traffic_df['date'].dt.normalize() < previous_end.normalize()) &
    (traffic_df['is_dead_sensor_day'] == False)
].copy()
...
staff_analysis = staff_df[
    (staff_df['date'].dt.normalize() >= analysis_start.normalize()) & 
    (staff_df['date'].dt.normalize() < analysis_end.normalize())
].copy()

staff_previous = staff_df[
    (staff_df['date'].dt.normalize() >= previous_start.normalize()) & 
    (staff_df['date'].dt.normalize() < previous_end.normalize())
].copy()
...
inventory_analysis = inventory_df[
    (inventory_df['week_starting'].dt.normalize() >= analysis_start.normalize()) & 
    (inventory_df['week_starting'].dt.normalize() < analysis_end.normalize())
].copy()

inventory_previous = inventory_df[
    (inventory_df['week_starting'].dt.normalize() >= previous_start.normalize()) & 
    (inventory_df['week_starting'].dt.normalize() < previous_end.normalize())
].copy()
