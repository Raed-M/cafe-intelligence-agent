...
# Define periods
analysis_start = pd.Timestamp('2026-04-20T00:00:00+03:00', tz='UTC').tz_convert('Asia/Riyadh')
analysis_end = pd.Timestamp('2026-04-27T00:00:00+03:00', tz='UTC').tz_convert('Asia/Riyadh')
...
baseline_periods = [
    ...
    (pd.Timestamp('2026-03-30T00:00:00+03:00', tz='UTC').tz_convert('Asia/Riyadh'),
     pd.Timestamp('2026-04-06T00:00:00+03:00', tz='UTC').tz_convert('Asia/Riyadh')),
    (pd.Timestamp('2026-03-23T00:00:00+03:00', tz='UTC').tz_convert('Asia/Riyadh'),
     pd.Timestamp('2026-03-30T00:00:00+03:00', tz='UTC').tz_convert('Asia/Riyadh'))
]
...
# Convert period timestamps to naive
analysis_start_naive = analysis_start.tz_localize(None)
analysis_end_naive = analysis_end.tz_localize(None)
baseline_periods_naive = [(p[0].tz_localize(None), p[1].tz_localize(None)) for p in baseline_periods]

# 1. Daily Revenue Anomaly Detection
print("Analyzing daily revenue...")

# Calculate daily revenue for analysis period
analysis_pos = pos_df[(pos_df['calendar_date_naive'] >= analysis_start_naive) & 
                      (pos_df['calendar_date_naive'] < analysis_end_naive)]
...
for period_start, period_end in baseline_periods_naive:
            ...
            findings.append({
                ...
                'metrics': {
                    ...
                    'baseline_mean_daily_revenue': {
                        ...
                        'numerator': None,
                        'denominator': None,
                        'period_start': baseline_periods_naive[0][0].isoformat(),
                        'period_end': baseline_periods_naive[-1][1].isoformat()
                    },
                ...
                'coverage_notes': [
                    f'Analysis period: {analysis_start_naive.date()} to {analysis_end_naive.date()}',
...
analysis_traffic = traffic_df_valid[(traffic_df_valid['date_naive'] >= analysis_start_naive) & 
                                    (traffic_df_valid['date_naive'] < analysis_end_naive)]
        ...
        for idx, row in analysis_traffic.iterrows():
            ...
            if abs(z_score) > 2.5:  # Higher threshold for hourly data
                findings.append({
                    ...
                    'coverage_notes': [
                        f'Analysis period: {analysis_start_naive.date()} to {analysis_end_naive.date()}',
                        f'Baseline: {len(baseline_traffic_combined)} hourly observations from 4 weeks prior',
...
if len(baseline_transaction_counts) > 1 and np.std(baseline_transaction_counts) > 0:
    ...
    for date, trans_count in analysis_transactions.items():
        ...
        if abs(z_score) > 2:
            findings.append({
                ...
                'coverage_notes': [
                    f'Analysis period: {analysis_start_naive.date()} to {analysis_end_naive.date()}',
                    f'Baseline: {len(baseline_transaction_counts)} daily observations from 4 weeks prior',
                    f'Threshold: |z-score| > 2.0'
                ],
                'assumptions': [
