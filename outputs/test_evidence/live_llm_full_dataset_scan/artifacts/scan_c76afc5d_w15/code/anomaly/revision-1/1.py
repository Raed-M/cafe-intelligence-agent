...
def calculate_daily_traffic(df, period_start, period_end):
    ...
    daily_traffic = period_data.groupby('date')['door_count'].sum()
...
findings.sort(key=lambda x: abs(x['metrics'].get('z_score', {}).get('value', 0) or x['metrics'].get('z_score_transactions', {}).get('value', 0) or x['metrics'].get('z_score_traffic', {}).get('value', 0)), reverse=True)
