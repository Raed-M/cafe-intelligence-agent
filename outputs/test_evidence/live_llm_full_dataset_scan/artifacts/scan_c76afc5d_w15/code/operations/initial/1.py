...
analysis_start = tz.localize(datetime(2026, 4, 20, 0, 0, 0))
analysis_end = tz.localize(datetime(2026, 4, 27, 0, 0, 0))
prev_start = tz.localize(datetime(2026, 4, 13, 0, 0, 0))
prev_end = tz.localize(datetime(2026, 4, 20, 0, 0, 0))
...
pos_df['timestamp_dt'] = pd.to_datetime(pos_df['timestamp'], utc=True).dt.tz_convert(tz)
traffic_df['date_dt'] = pd.to_datetime(traffic_df['date'], utc=True).dt.tz_convert(tz)
...
valid_traffic_analysis = traffic_analysis[traffic_analysis['is_dead_sensor_day'] == False]['door_count'].sum()
...
valid_transactions_prev = pos_prev[pos_prev['is_refund'] == False]['transaction_id'].nunique()
valid_traffic_prev = traffic_prev[traffic_prev['is_dead_sensor_day'] == False]['door_count'].sum()
...
if conversion_analysis is not None and conversion_prev is not None and traffic_analysis_valid_days >= 5:
    ...
    findings.append({
        ...
        "coverage_notes": [
            f"Analysis period: {traffic_analysis_valid_days} valid traffic days (dead sensor days excluded)",
            f"Previous period: {traffic_prev_valid_days} valid traffic days (dead sensor days excluded)",
...
# Convert staff dates
staff_df['date_dt'] = pd.to_datetime(staff_df['date'], utc=True).dt.tz_convert(tz)
...
revenue_prev = pos_prev[pos_prev['is_refund'] == False]['line_total_sar'].sum()
...
staff_analysis_days = staff_analysis['date_dt'].dt.date.nunique()
staff_prev_days = staff_prev['date_dt'].dt.date.nunique()
...
if labour_pct_analysis is not None and labour_pct_prev is not None and staff_analysis_days >= 5:
    ...
    findings.append({
        ...
        "sample_size": int(staff_analysis_days),
        "coverage_notes": [
...
inventory_df['week_starting_dt'] = pd.to_datetime(inventory_df['week_starting'], utc=True).dt.tz_convert(tz)
