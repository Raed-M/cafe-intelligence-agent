...
# Filter for products that were active during both periods
sku_comparison['launch_date'] = pd.to_datetime(sku_comparison['launch_date'], errors='coerce')
sku_comparison['retire_date'] = pd.to_datetime(sku_comparison['retire_date'], errors='coerce')
...
if len(sku_comparison_sorted) > 0:
    ...
    if abs(top_sku['revenue_change_pct']) > 10 and top_sku['revenue_analysis'] > 0:  # More than 10% change
        ...
        findings.append({
            ...
            "claim": f"Product {top_sku['item_en']} (SKU: {top_sku['sku']}) generated SAR {top_sku['revenue_analysis']:.2f} in the analysis period, compared to SAR {top_sku['revenue_previous']:.2f} in the previous period, representing a {top_sku['revenue_change_pct']:.1f}% change.",
            ...
            "assumptions": [
                ...
                "line_total_sar includes refunds as negative values",
                "Trailing baseline covers 4 weeks prior to analysis period"
            ],
            "confidence": 0.90
        })
