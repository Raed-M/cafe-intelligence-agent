...
if len(valid_items) > 0:
    ...
    units_sold_val = int(top_item['units_sold'])
    revenue_val = float(top_item['revenue_sar'])
    unit_cost_val = float(top_item['unit_cost_sar'])
    cogs_val = float(top_item['cogs_sar'])
    gross_profit_val = float(top_item['gross_profit_sar'])
    gross_margin_val = float(top_item['gross_margin_pct'])
    
    finding_1 = {
        "title": "Highest Gross Profit Item (Analysis Period)",
        "claim": f"{top_item['item_name']} generated an estimated {gross_profit_val:.2f} SAR gross profit (estimated {gross_margin_val:.1f}% margin) from {units_sold_val} units sold at {revenue_val:.2f} SAR revenue during 2026-05-11 to 2026-05-18, based on menu-declared unit cost of {unit_cost_val:.1f} SAR (no recipe/BOM verification available).",
        "finding_type": "item_economics",
        "metrics": {
            "units_sold": {
                "value": units_sold_val,
                "unit": "units",
                "numerator": None,
                "denominator": None,
            ...
            "cogs_sar": {
                "value": round(cogs_val, 2),
                "unit": "SAR",
                "numerator": round(float(units_sold_val), 0),
                "denominator": round(unit_cost_val, 1),
                "period_start": analysis_start,
                "period_end": analysis_end
            },
            "gross_profit_sar": {
                "value": round(gross_profit_val, 2),
                "unit": "SAR",
                "numerator": round(revenue_val, 2),
                "denominator": round(cogs_val, 2),
                "period_start": analysis_start,
                "period_end": analysis_end
            },
            "gross_margin_pct": {
                "value": round(gross_margin_val, 1),
                "unit": "%",
                "numerator": round(gross_profit_val, 2),
                "denominator": round(revenue_val, 2),
                "period_start": analysis_start,
                "period_end": analysis_end
            }
        },
        "source_names": ["pos.parquet", "menu.parquet"],
        "sample_size": units_sold_val,
        "coverage_notes": [
            ...
            f"Item {top_item['sku']} had {units_sold_val} transactions in period"
        ],
        "assumptions": [
...
if len(waste_items) > 0:
    ...
    total_waste_cost = float(waste_items['waste_cost_sar'].sum())
    total_units_wasted = int(waste_items['units_wasted'].sum())
    ...
    for idx, row in waste_ranking.iterrows():
        ranking_list.append({
            'item': str(row['item']),
            'waste_cost_sar': round(float(row['waste_cost_sar']), 2)
        })
    
    # Convert top_waste values to native Python types
    top_waste_units = int(top_waste['units_wasted'])
    top_waste_unit_cost = float(top_waste['unit_cost_sar'])
    top_waste_cost = float(top_waste['waste_cost_sar'])
    top_waste_item = str(top_waste['item'])
    
    finding_3 = {
        "title": "Highest Waste Cost Item (Analysis Period)",
        "claim": f"{top_waste_item} had the highest waste cost of {top_waste_cost:.2f} SAR from {top_waste_units} units wasted at {top_waste_unit_cost:.1f} SAR/unit during 2026-05-11 to 2026-05-18. Total waste cost across {len(waste_items)} items: {total_waste_cost:.2f} SAR ({total_units_wasted} units).",
        "finding_type": "waste_cost",
        "metrics": {
            "units_wasted": {
                "value": top_waste_units,
                "unit": "units",
                "numerator": None,
                "denominator": None,
                "period_start": analysis_start,
            ...
            "unit_cost_sar": {
                "value": round(top_waste_unit_cost, 1),
                "unit": "SAR/unit",
                "numerator": None,
                "denominator": None,
                "period_start": analysis_start,
            ...
            "waste_cost_sar": {
                "value": round(top_waste_cost, 2),
                "unit": "SAR",
                "numerator": top_waste_units,
                "denominator": round(top_waste_unit_cost, 1),
                "period_start": analysis_start,
                "period_end": analysis_end
            },
        ...
        "sample_size": total_units_wasted,
        "coverage_notes": [
            f"Analysis period: 2026-05-11 to 2026-05-18",
