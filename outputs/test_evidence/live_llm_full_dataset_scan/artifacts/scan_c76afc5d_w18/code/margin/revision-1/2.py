...
# Parse analysis period boundaries with timezone awareness
...
# Convert naive datetimes to UTC-aware for comparison
def ensure_tz_aware(dt_series, tz='UTC'):
    """Convert naive datetime to timezone-aware if needed."""
    if dt_series.dt.tz is None:
        return dt_series.dt.tz_localize(tz)
    return dt_series

pos_df['timestamp'] = ensure_tz_aware(pos_df['timestamp'])
inventory_df['week_starting'] = ensure_tz_aware(inventory_df['week_starting'])
menu_df['launch_date'] = ensure_tz_aware(menu_df['launch_date'])
menu_df['retire_date'] = ensure_tz_aware(menu_df['retire_date'])
emails_df['date'] = ensure_tz_aware(emails_df['date'])
emails_df['effective_date'] = ensure_tz_aware(emails_df['effective_date'])
...
if len(item_metrics_valid) > 0:
    ...
    finding_1 = {
        "title": "High Gross Profit Item in Analysis Period",
        ...
        "coverage_notes": [
            ...
            f"Item {top_item['item_name']} (SKU: {top_item['sku']}) ranked among {len(item_metrics_valid)} items with valid costs"
        ],
        "assumptions": [
...
if len(price_changes) > 0:
    # Ensure effective_date is timezone-aware for comparison
    price_changes['effective_date'] = ensure_tz_aware(price_changes['effective_date'])
    ...
    if len(price_changes_relevant) > 0:
        ...
        finding_2 = {
            ...
            "claim": f"Supplier price for {top_change['entity_or_ingredient']} changed from {top_change['old_price']:.2f} {top_change['currency']}/{top_change['unit']} to {top_change['new_price']:.2f} {top_change['currency']}/{top_change['unit']} (change: {top_change['pct_change']:.2f}%), effective {pd.to_datetime(top_change['effective_date']).strftime('%Y-%m-%d')}. This represents a {abs(top_change['price_delta']):.2f} {top_change['currency']} per-unit cost shift.",
        ...
        finding_3 = {
            ...
            "metrics": {
                ...
                "waste_cost_sar": {
                    ...
                    "denominator": round(top_waste['unit_cost_sar'], 2),
                    "period_start": analysis_start,
                    "period_end": analysis_end
                },
                "unit_cost_sar": {
            ...
            "coverage_notes": [
                f"Inventory records for week starting {pd.to_datetime(top_waste['week_starting']).strftime('%Y-%m-%d')}",
