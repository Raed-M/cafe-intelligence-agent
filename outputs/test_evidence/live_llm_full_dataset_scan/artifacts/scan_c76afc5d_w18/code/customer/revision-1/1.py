...
if len(reviews_analysis) > 0:
    ...
    finding_3 = {
        ...
        "claim": f"Of {total_reviews} reviews in the analysis period, {english_count} ({english_pct:.1f}%) are in English and {arabic_count} ({arabic_pct:.1f}%) are in Arabic. Average rating for English reviews: {en_avg_rating:.2f if en_avg_rating else 'N/A'} stars; Arabic reviews: {ar_avg_rating:.2f if ar_avg_rating else 'N/A'} stars.",
        ...
        "metrics": {
            ...
            "english_avg_rating": {
                "value": round(en_avg_rating, 2) if en_avg_rating else None,
                "unit": "stars",
                "numerator": round(en_avg_rating * english_count, 2) if en_avg_rating else None,
                "denominator": english_count if english_count > 0 else None,
                "period_start": analysis_start + "T00:00:00+03:00",
                "period_end": analysis_end + "T00:00:00+03:00"
            },
            "arabic_avg_rating": {
                "value": round(ar_avg_rating, 2) if ar_avg_rating else None,
                "unit": "stars",
                "numerator": round(ar_avg_rating * arabic_count, 2) if ar_avg_rating else None,
