...
# Read artifacts using pandas only
...
def get_hourly_traffic(df, period_start, period_end):
    ...
    # Convert period_start and period_end to dates for comparison with df['date']
    period_start_date = pd.Timestamp(period_start).date()
    period_end_date = pd.Timestamp(period_end).date()
    
    # Convert df['date'] to date objects for comparison
...
if len(baseline_hourly_traffic_list) > 0 and len(analysis_hourly_traffic) > 0:
    ...
    if anomalies:
        ...
        findings.append({
            ...
            "coverage_notes": [
                f"Analysis period: {analysis_period['start'].date()} to {analysis_period['end'].date()}",
                f"Baseline: hourly averages from 4 trailing weeks ({trailing_baseline_periods[0]['start'].date()} to {trailing_baseline_periods[-1]['end'].date()})",
