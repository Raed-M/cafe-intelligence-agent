        ...
        for idx, row in daily_revenue_analysis.iterrows():
            ...
            if abs(z_score) > 2.0:  # 2 standard deviations
                findings.append({
                    ...
                    'claim': f"Daily revenue of {row['revenue']:.2f} SAR on {row['date']} is {abs(z_score):.2f} standard deviations from baseline mean of {baseline_mean:.2f} SAR",
                    ...
                    'metrics': {
                        ...
                        'baseline_mean_daily_revenue': {
                            ...
                            'numerator': None,
                            'denominator': None,
                            'period_start': '2025-12-29T00:00:00+03:00',
...
if len(baseline_daily_txns) > 0 and len(daily_txn_analysis) > 0:
    ...
    if baseline_txn_std > 0:
        ...
        for idx, row in daily_txn_analysis.iterrows():
            ...
            if abs(z_score) > 2.0:  # 2 standard deviations
                findings.append({
                    'title': f"Daily Transaction Count Anomaly on {row['date']}",
                    'claim': f"Daily transaction count of {row['transaction_count']} on {row['date']} is {abs(z_score):.2f} standard deviations from baseline mean of {baseline_txn_mean:.1f}",
