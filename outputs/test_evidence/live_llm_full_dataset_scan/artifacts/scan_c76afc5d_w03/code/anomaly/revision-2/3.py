    ...
    if baseline_std > 0:
        ...
        for idx, row in daily_revenue_analysis.iterrows():
            ...
            if abs(z_score) > 2.0:  # 2 standard deviations
                ...
                period_start_iso = date_obj.strftime('%Y-%m-%dT00:00:00+03:00')
                period_end_iso = date_obj.strftime('%Y-%m-%dT23:59:59+03:00')
...
# Filter traffic for analysis period - convert date column to timestamp for comparison
