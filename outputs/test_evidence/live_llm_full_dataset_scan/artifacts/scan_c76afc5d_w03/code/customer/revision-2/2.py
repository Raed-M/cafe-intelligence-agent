...
if len(analysis_reviews) > 0:
    ...
    if len(reviews_with_text) > 0:
        ...
        finding3 = {
            ...
            "claim": f"Among {len(analysis_reviews)} reviews in the analysis period (Jan 26 - Feb 2, 2026), {len(reviews_with_text)} contained text content. Keyword analysis identified topics in {len(reviews_with_topics)} of these reviews. The most frequently mentioned topics were: service ({service_count} mentions), taste ({taste_count} mentions), and quality ({quality_count} mentions). {reviews_without_topics} reviews contained no extractable topics using keyword matching. Note: {len(analysis_reviews) - len(reviews_with_text)} reviews had no text content and were excluded. Analysis covered both English and Arabic text.",
            ...
            "metrics": {
                ...
                "reviews_without_topics": {
                    "value": reviews_without_topics,
                    "unit": "count",
                    "numerator": reviews_without_topics,
                    "denominator": len(reviews_with_text),
                    "period_start": analysis_start,
                    "period_end": analysis_end
                },
                "service_mentions": {
                    ...
                    "denominator": len(reviews_with_topics) if len(reviews_with_topics) > 0 else len(reviews_with_text),
                    "period_start": analysis_start,
                    "period_end": analysis_end
                },
                "taste_mentions": {
            ...
            "coverage_notes": [
                ...
                f"Reviews without extractable topics: {reviews_without_topics}",
