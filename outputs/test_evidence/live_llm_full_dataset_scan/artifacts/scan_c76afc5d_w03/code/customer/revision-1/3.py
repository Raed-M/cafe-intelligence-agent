...
if total_reviews_period > 0:
    finding1 = {
        ...
        "claim": f"During the analysis period (Jan 26 - Feb 2, 2026), {total_reviews_period} reviews were collected with an average rating of {avg_rating:.2f} out of 5. Rating distribution: {dict((int(k), int(v)) for k, v in rating_counts.items())}",
        ...
        "metrics": {
            "average_rating": {
                ...
                "denominator": int(total_reviews_period),
                "period_start": analysis_start,
                "period_end": analysis_end
            },
            "total_reviews": {
                "value": int(total_reviews_period),
                "unit": "count",
                "numerator": None,
                "denominator": None,
                "period_start": analysis_start,
        ...
        "source_names": [str(s) for s in reviews_analysis['source'].unique()],
        "sample_size": int(total_reviews_period),
        "coverage_notes": [
            f"Total reviews in analysis period: {total_reviews_period}",
            f"Sources covered: {', '.join(str(s) for s in reviews_analysis['source'].unique())}",
            f"Language distribution: {dict((str(k), int(v)) for k, v in reviews_analysis['language'].value_counts().items())}"
        ],
        "assumptions": [
...
if total_reviews_period > 0 and len(top_topics) > 0:
    ...
    for topic, count in top_topics:
        topic_parts.append(f"{topic} ({int(count)} mentions)")
    ...
    for i, (topic, count) in enumerate(top_topics):
        metrics_dict[f"{topic}_mentions"] = {
            ...
            "unit": "mentions",
            "numerator": int(count),
            "denominator": int(total_with_topics),
            "period_start": analysis_start,
            "period_end": analysis_end
        }
    
    metrics_dict["reviews_with_topics"] = {
        "value": int(total_with_topics),
        "unit": "count",
        "numerator": int(total_with_topics),
        "denominator": int(total_reviews_period),
        "period_start": analysis_start,
        "period_end": analysis_end
    }
    ...
    finding2 = {
        ...
        "source_names": [str(s) for s in (reviews_with_topics['source'].unique() if total_with_topics > 0 else reviews_analysis['source'].unique())],
        "sample_size": int(total_with_topics),
        "coverage_notes": [
            ...
            f"Language coverage: {dict((str(k), int(v)) for k, v in reviews_with_topics['language'].value_counts().items()) if total_with_topics > 0 else 'N/A'}"
        ],
        "assumptions": [
