...
# Ensure timezone-aware comparisons for pos_df
if pos_df['timestamp_local'].dt.tz is None:
    pos_df['timestamp_local'] = pos_df['timestamp_local'].dt.tz_localize('UTC+03:00')
    ...
    for date, revenue in analysis_daily_revenue.items():
        ...
        if abs(z_score) > 2.0:
            findings.append({
                ...
                "coverage_notes": [
                    f"Analysis period: 2026-05-04 to 2026-05-11",
                    f"Baseline: 2026-04-06 to 2026-05-04 (4 weeks)",
        ...
        findings.append({
            ...
            "metrics": {
                "observed_hourly_door_count": {
                    ...
                    "numerator": None,
                    "denominator": None,
                    "period_start": f"{top_anomaly['date']}T{top_anomaly['hour']:02d}:00:00+03:00",
                    "period_end": f"{top_anomaly['date']}T{top_anomaly['hour']:02d}:59:59+03:00"
                },
            ...
            if baseline_waste_std > 0:
                ...
                if abs(z_score_waste) > 2.0:
                    findings.append({
                        ...
                        "coverage_notes": [
                            f"Analysis period: week starting 2026-05-04",
                            f"Baseline: 4 weeks from 2026-04-06 to 2026-05-04",
