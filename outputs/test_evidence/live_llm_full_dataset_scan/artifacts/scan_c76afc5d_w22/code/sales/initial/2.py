...
def main():
    ...
    if len(analysis_data) > 0 and len(trailing_data_list) > 0:
        ...
        trailing_top_rev = float(trailing_avg_revenue.get(top_category, 0))
        
        top_change = analysis_top_rev - trailing_top_rev
        top_pct_change = (top_change / trailing_top_rev * 100) if trailing_top_rev != 0 else 0
        
        analysis_top_txns = int(analysis_category_txns.get(top_category, 0))
        trailing_top_txns = float(trailing_avg_txns.get(top_category, 0))
    ...
    if len(analysis_data) > 0 and len(previous_data) > 0:
        ...
        for channel in all_channels:
            analysis_rev = float(analysis_channel_revenue.get(channel, 0))
            previous_rev = float(previous_channel_revenue.get(channel, 0))
            change = analysis_rev - previous_rev
            channel_changes[channel] = {
                'analysis_rev': analysis_rev,
                'previous_rev': previous_rev,
                'change': change,
                'analysis_txns': int(analysis_channel_txns.get(channel, 0)),
                'previous_txns': int(previous_channel_txns.get(channel, 0))
            }
        
        # Get channel with largest absolute change
        max_channel = max(channel_changes.items(), key=lambda x: abs(x[1]['change']))
        channel_name = str(max_channel[0])
