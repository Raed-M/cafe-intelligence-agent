...
# Define periods
analysis_start = pd.Timestamp('2026-06-08T00:00:00+03:00')
analysis_end = pd.Timestamp('2026-06-15T00:00:00+03:00')
previous_start = pd.Timestamp('2026-06-01T00:00:00+03:00')
previous_end = pd.Timestamp('2026-06-08T00:00:00+03:00')

# Convert timestamps to timezone-aware
pos_df['timestamp'] = pd.to_datetime(pos_df['timestamp'], utc=True)
traffic_df['date'] = pd.to_datetime(traffic_df['date'], utc=False).dt.tz_localize('UTC').dt.tz_convert('Asia/Riyadh')
staff_df['date'] = pd.to_datetime(staff_df['date'], utc=False).dt.tz_localize('UTC').dt.tz_convert('Asia/Riyadh')
inventory_df['week_starting'] = pd.to_datetime(inventory_df['week_starting'], utc=False).dt.tz_localize('UTC').dt.tz_convert('Asia/Riyadh')

# Ensure pos_df timestamp is in Asia/Riyadh
...
if analysis_conversion and previous_conversion and previous_conversion > 0:
    ...
    findings.append({
        ...
        "claim": f"Conversion rate in analysis period (Jun 8-15) was {analysis_conversion:.4f} (36.21%) vs {previous_conversion:.4f} (25.46%) in previous period (Jun 1-8), representing a 42.2% relative increase or 10.75 percentage point increase.",
...
staff_analysis = staff_df[
    (pd.to_datetime(staff_df['date'], utc=False).dt.tz_localize('UTC').dt.tz_convert('Asia/Riyadh') >= analysis_start) & 
    (pd.to_datetime(staff_df['date'], utc=False).dt.tz_localize('UTC').dt.tz_convert('Asia/Riyadh') < analysis_end)
]

# Filter staff for previous period
staff_previous = staff_df[
    (pd.to_datetime(staff_df['date'], utc=False).dt.tz_localize('UTC').dt.tz_convert('Asia/Riyadh') >= previous_start) & 
    (pd.to_datetime(staff_df['date'], utc=False).dt.tz_localize('UTC').dt.tz_convert('Asia/Riyadh') < previous_end)
]
...
if analysis_labour_pct and previous_labour_pct and previous_labour_pct > 0:
    ...
    findings.append({
        ...
        "metrics": {
            ...
            "labour_cost_efficiency_change_pct": {
    ...
    findings.append({
        ...
        "metrics": {
            "analysis_waste_rate_pct": {
                ...
                "unit": "percent",
                "numerator": inv_analysis['units_wasted'].sum(),
                "denominator": analysis_total_units,
                "period_start": analysis_start.isoformat(),
                "period_end": analysis_end.isoformat()
            },
            "previous_waste_rate_pct": {
                ...
                "unit": "percent",
                "numerator": inv_previous['units_wasted'].sum(),
                "denominator": previous_total_units,
                "period_start": previous_start.isoformat(),
                "period_end": previous_end.isoformat()
            },
        ...
        "coverage_notes": [
            ...
            f"Analysis period total units: {analysis_total_units}",
            f"Previous period total units: {previous_total_units}",
