...
# Convert timestamp to datetime and make timezone-naive for comparison
pos_df['timestamp'] = pd.to_datetime(pos_df['timestamp']).dt.tz_localize(None)
...
pos_analysis = pos_df[
    (pos_df['timestamp'] >= analysis_start_naive) & 
    (pos_df['timestamp'] < analysis_end_naive)
].copy()
...
# Get top 1 item
top_1_item = item_metrics_sorted.head(1)
...
# Convert inventory week_starting to datetime and make timezone-naive
inventory_df['week_starting_dt'] = pd.to_datetime(inventory_df['week_starting']).dt.tz_localize(None)

# Filter inventory to analysis period
inventory_analysis = inventory_df[
    (inventory_df['week_starting_dt'] >= analysis_start_naive) &
    (inventory_df['week_starting_dt'] < analysis_end_naive)
].copy()
...
# Convert emails date to datetime and make timezone-naive
emails_df['date_dt'] = pd.to_datetime(emails_df['date']).dt.tz_localize(None)

# Filter emails to analysis period and price changes
emails_analysis = emails_df[
    (emails_df['date_dt'] >= analysis_start_naive) &
    (emails_df['date_dt'] < analysis_end_naive) &
...
# Finding 1: Top item by gross profit
if len(top_1_item) > 0:
    top_1 = top_1_item.iloc[0]
