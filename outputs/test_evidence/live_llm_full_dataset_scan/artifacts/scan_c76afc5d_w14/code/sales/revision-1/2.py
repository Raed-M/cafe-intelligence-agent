...
for _, menu_row in menu_df.iterrows():
    sku = menu_row['sku']
    launch_date = pd.to_datetime(menu_row['launch_date'], utc=True) if pd.notna(menu_row['launch_date']) else None
    retire_date = pd.to_datetime(menu_row['retire_date'], utc=True) if pd.notna(menu_row['retire_date']) else None
