...
# Define analysis periods (using UTC offset +03:00 for Asia/Riyadh)
analysis_start = pd.Timestamp('2026-04-13', tz='UTC').tz_convert('UTC') + pd.Timedelta(hours=3)
analysis_end = pd.Timestamp('2026-04-20', tz='UTC').tz_convert('UTC') + pd.Timedelta(hours=3)
prev_start = pd.Timestamp('2026-04-06', tz='UTC').tz_convert('UTC') + pd.Timedelta(hours=3)
prev_end = pd.Timestamp('2026-04-13', tz='UTC').tz_convert('UTC') + pd.Timedelta(hours=3)
...
# Convert traffic dates to datetime for consistent comparison
traffic_df['date'] = pd.to_datetime(traffic_df['date'])
...
traffic_prev = traffic_df[(traffic_df['date'] >= pd.Timestamp(prev_start.date())) & (traffic_df['date'] < pd.Timestamp(prev_end.date()))].copy()

staff_analysis = staff_df[(staff_df['date'] >= pd.Timestamp(analysis_start.date())) & (staff_df['date'] < pd.Timestamp(analysis_end.date()))].copy()
staff_prev = staff_df[(staff_df['date'] >= pd.Timestamp(prev_start.date())) & (staff_df['date'] < pd.Timestamp(prev_end.date()))].copy()
        ...
        findings.append({
            ...
            "metrics": {
                "conversion_rate_analysis": {
                    ...
                    "denominator": int(valid_traffic_analysis),
            ...
            "coverage_notes": [
                f"Analysis period: {unique_transactions_analysis} valid transactions from {int(valid_traffic_analysis)} valid footfall events",
                f"Previous period: {unique_transactions_prev} valid transactions from {int(valid_traffic_prev)} valid footfall events",
        ...
        findings.append({
            ...
            "metrics": {
                ...
                "waste_pct_analysis": {
                    ...
                    "unit": "percent",
                    "numerator": int(inventory_analysis['units_wasted'].sum()),
                    "denominator": int(total_units_ordered_analysis),
                    "period_start": analysis_start.isoformat(),
                    "period_end": analysis_end.isoformat()
                },
                "waste_pct_previous": {
                    ...
                    "unit": "percent",
                    "numerator": int(inventory_prev['units_wasted'].sum()),
                    "denominator": int(total_units_ordered_prev),
                    "period_start": prev_start.isoformat(),
                    "period_end": prev_end.isoformat()
                },
                "units_sold_analysis": {
