...
# Convert launch_date and retire_date to timezone-aware datetime for comparison
# Handle both tz-aware and tz-naive dates by converting to tz-aware
def make_tz_aware(dt_series):
    """Convert datetime series to timezone-aware (UTC) for comparison"""
    result = pd.to_datetime(dt_series, utc=True)
    return result

analysis_sku_revenue['launch_date_tz'] = make_tz_aware(analysis_sku_revenue['launch_date'])
analysis_sku_revenue['retire_date_tz'] = make_tz_aware(analysis_sku_revenue['retire_date'])
previous_sku_revenue['launch_date_tz'] = make_tz_aware(previous_sku_revenue['launch_date'])
previous_sku_revenue['retire_date_tz'] = make_tz_aware(previous_sku_revenue['retire_date'])

# Convert analysis_start and analysis_end to UTC for comparison
...
analysis_sku_revenue['is_active'] = (
    (analysis_sku_revenue['launch_date_tz'].isna() | (analysis_sku_revenue['launch_date_tz'] <= analysis_start_utc)) &
    (analysis_sku_revenue['retire_date_tz'].isna() | (analysis_sku_revenue['retire_date_tz'] > analysis_start_utc))
)

previous_sku_revenue['is_active'] = (
    (previous_sku_revenue['launch_date_tz'].isna() | (previous_sku_revenue['launch_date_tz'] <= previous_start_utc)) &
    (previous_sku_revenue['retire_date_tz'].isna() | (previous_sku_revenue['retire_date_tz'] > previous_start_utc))
)
