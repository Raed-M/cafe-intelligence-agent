...
if total_reviews > 0:
    ...
    finding2 = {
        ...
        "claim": f"Of {total_reviews} reviews in the analysis period, {int(language_counts.get('en', 0))} are in English and {int(language_counts.get('ar', 0))} are in Arabic.",
...
if total_reviews > 0:
    ...
    finding3 = {
        ...
        "claim": f"Reviews in the analysis period come from {len(source_counts)} sources: {', '.join([f'{source} ({int(count)})' for source, count in source_counts.items()])}.",
        "finding_type": "source_coverage",
        "metrics": {},
        "source_names": source_names,
        "sample_size": int(total_reviews),
        "coverage_notes": [
            f"Source distribution: {dict((k, int(v)) for k, v in source_counts.items())}",
