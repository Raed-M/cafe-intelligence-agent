...
if len(reviews_analysis) > 0:
    ...
    finding_2 = {
        ...
        "sample_size": len(reviews_analysis),
        "coverage_notes": [
            f"English reviews: {len(english_reviews)} ({100*len(english_reviews)/len(reviews_analysis):.1f}% of total)",
            f"Arabic reviews: {len(arabic_reviews)} ({100*len(arabic_reviews)/len(reviews_analysis):.1f}% of total)",
