...
if len(emails_df) > 0:
    ...
    if len(price_changes) > 0:
        ...
        # Safely extract facts - handle array case
        facts_value = top_change['facts']
        if isinstance(facts_value, np.ndarray):
            facts_text = str(facts_value[0]) if len(facts_value) > 0 and pd.notna(facts_value[0]) else ""
        else:
            facts_text = str(facts_value) if pd.notna(facts_value) else ""
        
        # Safely extract confidence
        confidence_value = top_change['confidence']
        if isinstance(confidence_value, np.ndarray):
            confidence_num = float(confidence_value[0]) if len(confidence_value) > 0 and pd.notna(confidence_value[0]) else 0.75
        else:
            confidence_num = float(confidence_value) if pd.notna(confidence_value) else 0.75
        ...
        finding_3 = {
            ...
            "coverage_notes": [
                ...
                f"Email extraction confidence: {confidence_num}",
