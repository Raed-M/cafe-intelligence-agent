...
# Helper function to convert numpy int64 to Python int
...
footfall_analysis = int(traffic_analysis_valid['door_count'].sum())
...
footfall_previous = int(traffic_previous_valid['door_count'].sum())
...
staff_hours_analysis = float(staff_analysis['computed_duration_hours'].sum())
...
staff_hours_previous = float(staff_previous['computed_duration_hours'].sum())
...
waste_cost_analysis = float(inventory_analysis['known_waste_cost_sar'].sum())
...
waste_cost_previous = float(inventory_previous['known_waste_cost_sar'].sum())
...
if conversion_analysis is not None and conversion_previous is not None and footfall_analysis > 0 and footfall_previous > 0:
    findings.append({
        ...
        "coverage_notes": [
            f"Analysis period: {footfall_analysis} valid footfall records (dead sensor days excluded)",
            f"Previous period: {footfall_previous} valid footfall records (dead sensor days excluded)",
            f"Analysis period: {valid_transactions_analysis} non-refund transactions",
            f"Previous period: {valid_transactions_previous} non-refund transactions"
        ],
        "assumptions": [
...
with open(output_path, 'w') as f:
    json.dump(result, f, indent=2, default=to_serializable)
