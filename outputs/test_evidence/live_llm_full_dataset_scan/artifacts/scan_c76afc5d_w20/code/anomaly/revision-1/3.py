...
if len(baseline_daily_revenues) >= 5 and np.std(baseline_daily_revenues) > 0:
    ...
    if anomalies:
        ...
        date_str = top_anomaly['date'].strftime('%Y-%m-%d')
        next_date_str = (top_anomaly['date'] + timedelta(days=1)).strftime('%Y-%m-%d')
        
        finding = {
            "title": "Unusual Daily Revenue Detected",
            "claim": f"Daily revenue on {date_str} was {top_anomaly['revenue']:.2f} SAR, {abs(top_anomaly['z_score']):.2f} standard deviations from baseline mean of {baseline_mean:.2f} SAR.",
            ...
            "coverage_notes": [
                f"Baseline computed from {len(baseline_daily_revenues)} daily observations across 4 trailing weeks (2026-04-27 to 2026-05-25)",
...
if len(baseline_daily_transactions) >= 5 and np.std(baseline_daily_transactions) > 0:
    ...
    if trans_anomalies and len(findings) < 3:
        ...
        date_str = top_trans_anomaly['date'].strftime('%Y-%m-%d')
        next_date_str = (top_trans_anomaly['date'] + timedelta(days=1)).strftime('%Y-%m-%d')
        
        finding = {
            "title": "Unusual Daily Transaction Count Detected",
            "claim": f"Daily transaction count on {date_str} was {top_trans_anomaly['transaction_count']} transactions, {abs(top_trans_anomaly['z_score']):.2f} standard deviations from baseline mean of {baseline_trans_mean:.1f} transactions.",
            ...
            "coverage_notes": [
                f"Baseline computed from {len(baseline_daily_transactions)} daily observations across 4 trailing weeks (2026-04-27 to 2026-05-25)",
...
if len(baseline_daily_traffic) >= 5 and np.std(baseline_daily_traffic) > 0:
    ...
    if traffic_anomalies and len(findings) < 3:
        ...
        date_str = top_traffic_anomaly['date'].strftime('%Y-%m-%d')
        next_date_str = (top_traffic_anomaly['date'] + timedelta(days=1)).strftime('%Y-%m-%d')
        
        finding = {
            "title": "Unusual Daily Traffic Count Detected",
            "claim": f"Daily foot traffic on {date_str} was {top_traffic_anomaly['traffic_count']} visitors, {abs(top_traffic_anomaly['z_score']):.2f} standard deviations from baseline mean of {baseline_traffic_mean:.1f} visitors.",
            ...
            "coverage_notes": [
                f"Baseline computed from {len(baseline_daily_traffic)} daily observations across 4 trailing weeks (2026-04-27 to 2026-05-25)",
