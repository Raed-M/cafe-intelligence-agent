...
# Ensure timezone-aware comparison: convert analysis dates to naive UTC for comparison with naive datetimes
...
inventory_analysis = inventory_df[(inventory_df['week_starting'] >= analysis_start_naive) & (inventory_df['week_starting'] < analysis_end_naive)]
...
records_excluded = total_inventory_records - waste_records
...
if waste_records > 0:
    ...
    finding_2 = {
        ...
        "claim": f"Total known waste cost for the analysis week (2026-01-12 to 2026-01-19) is {total_waste_cost:.2f} SAR across {waste_records} items with recorded waste observations out of {total_inventory_records} total inventory records in the period. This represents waste from {int(inventory_with_waste['units_wasted'].sum())} units across {waste_records} SKUs. {records_excluded} inventory records had null/blank waste values and were excluded per methodology.",
        ...
        "coverage_notes": [
            f"Analysis includes {waste_records} items with non-null, positive waste_cost_sar values",
            f"Total inventory records for period: {total_inventory_records}",
            f"Records excluded due to null/blank waste values: {records_excluded}",
            "Known waste cost represents actual loss value from inventory.known_waste_cost_sar field",
            "Includes only waste_cost_sar values >= 0 with non-null entries; excludes estimated, pending, or flagged waste"
        ],
        "assumptions": [
            "Waste cost represents actual verified loss value",
            ...
            "Analysis scope limited to records with explicit waste cost entries"
        ],
        "confidence": 0.90
    }
...
if len(price_changes) > 0:
    ...
    finding_3 = {
        ...
        "claim": f"Email analysis detected {len(price_changes)} price change notification(s) in the analysis period. The largest reported change is for {top_change['entity_or_ingredient']} from {top_change['old_price']:.2f} to {top_change['new_price']:.2f} {top_change['currency']}/{top_change['unit']}, representing a {top_change['price_change_pct']:.2f}% change, effective {top_change['effective_date'].strftime('%Y-%m-%d') if pd.notna(top_change['effective_date']) else 'date not specified'}. Business impact cannot be quantified without recipe/BOM data showing usage per drink and standing order quantities from procurement records.",
        ...
        "coverage_notes": [
            f"Email analysis identified {len(price_changes)} price change notification(s) in the analysis period",
            ...
            "Extraction not independently verified against official supplier documentation"
        ],
        "assumptions": [
            "Email extraction captured old and new prices from supplier communications",
            "Price changes are from supplier communications (not verified against invoices or official price lists)",
            ...
            "Email-extracted prices may not represent binding commitments"
        ],
        "confidence": 0.70
    }
