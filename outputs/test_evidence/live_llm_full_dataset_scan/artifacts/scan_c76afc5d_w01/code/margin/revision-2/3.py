...
# Parse analysis period with timezone awareness
analysis_start_dt = pd.to_datetime(analysis_start)
analysis_end_dt = pd.to_datetime(analysis_end)

# Ensure pos_df timestamp is timezone-aware; if not, localize to UTC+03:00
if pos_df['timestamp'].dt.tz is None:
    pos_df['timestamp'] = pos_df['timestamp'].dt.tz_localize('UTC+03:00')
else:
    # Convert to UTC+03:00 if it has a different timezone
    pos_df['timestamp'] = pos_df['timestamp'].dt.tz_convert('UTC+03:00')

# Ensure inventory_df week_starting is timezone-aware; if not, localize to UTC+03:00
if inventory_df['week_starting'].dt.tz is None:
    inventory_df['week_starting'] = inventory_df['week_starting'].dt.tz_localize('UTC+03:00')
else:
    inventory_df['week_starting'] = inventory_df['week_starting'].dt.tz_convert('UTC+03:00')

# Filter POS to analysis period (both are now timezone-aware)
pos_analysis = pos_df[(pos_df['timestamp'] >= analysis_start_dt) & (pos_df['timestamp'] < analysis_end_dt)].copy()

# Filter inventory to analysis period (both are now timezone-aware)
...
# Total inventory records in period before filtering
...
# Count records with null/blank waste_cost_sar
...
if len(inventory_with_waste) > 0:
    ...
    finding_2 = {
        ...
        "coverage_notes": [
            f"Total inventory records in analysis period before filtering: {total_inventory_records}",
            f"Records with non-null waste_cost_sar (known waste): {waste_items_count}",
            f"Records with null/blank waste_cost_sar (unknown waste): {null_waste_records}",
        ...
        "assumptions": [
            ...
            "Unknown waste (null waste_cost_sar values) are excluded from this 'known waste cost' calculation"
        ],
        "confidence": 0.75
    }
...
if len(price_changes) > 0:
    ...
    finding_3 = {
        ...
        "claim": f"Email evidence indicates roasted coffee price change from {old_price:.2f} to {new_price:.2f} SAR/{unit} ({price_change_pct:+.2f}%), effective {effective_date.strftime('%Y-%m-%d')}. No recipe/BOM, standing order volume, or independent corroboration available to calculate per-drink or procurement cost impact.",
        ...
        "coverage_notes": [
            ...
            "No confirmation that this supplier is the cafe's actual roasted coffee vendor"
        ],
        "assumptions": [
            ...
            "Effective date (2026-04-15) represents a confirmed change, not a proposal"
        ],
        "confidence": 0.45
    }
