...
# Category mix analysis (excluding refunds)
...
# Channel mix analysis (excluding refunds)
...
# Finding 4: Category Mix Analysis (excluding refunds)
if len(analysis_category_revenue) > 0 and len(previous_category_revenue) > 0:
    analysis_total_revenue = analysis_metrics['total_revenue']
    previous_total_revenue = previous_metrics['total_revenue']
    
    # Get the top category by revenue for both periods
    top_category_analysis = analysis_category_revenue.idxmax()
    top_category_previous = previous_category_revenue.idxmax()
    
    top_revenue_analysis = analysis_category_revenue[top_category_analysis]
    top_revenue_previous = previous_category_revenue[top_category_previous]
    
    analysis_mix_pct = (top_revenue_analysis / analysis_total_revenue * 100) if analysis_total_revenue > 0 else 0
    previous_mix_pct = (top_revenue_previous / previous_total_revenue * 100) if previous_total_revenue > 0 else 0
    
    mix_shift = analysis_mix_pct - previous_mix_pct
    
    if mix_shift != 0:
        findings.append({
            'title': 'Category Revenue Mix Shift Week-over-Week',
            'claim': f'Top category ({top_category_analysis}) revenue mix shifted from {previous_mix_pct:.1f}% to {analysis_mix_pct:.1f}% ({mix_shift:+.1f} percentage points)',
            'finding_type': 'category_mix',
            'metrics': {
                'analysis_period_category_revenue': {
                    'value': round(top_revenue_analysis, 2),
                    'unit': 'SAR',
                    'numerator': None,
                    'denominator': None,
                    'period_start': analysis_period['start'].isoformat(),
                ...
                'previous_period_category_revenue': {
                    'value': round(top_revenue_previous, 2),
                    'unit': 'SAR',
                    'numerator': None,
                    'denominator': None,
                    'period_start': previous_period['start'].isoformat(),
                ...
                'analysis_period_category_mix_pct': {
                    'value': analysis_mix_pct,
                    'unit': '%',
                    'numerator': round(top_revenue_analysis, 2),
                    'denominator': round(analysis_total_revenue, 2),
                    'period_start': analysis_period['start'].isoformat(),
                    'period_end': analysis_period['end'].isoformat()
                },
                'previous_period_category_mix_pct': {
                    'value': previous_mix_pct,
                    'unit': '%',
                    'numerator': round(top_revenue_previous, 2),
            ...
            'sample_size': analysis_metrics['line_items'],
            'coverage_notes': [
                f'Analysis period: {analysis_metrics["line_items"]} line items, SAR {analysis_total_revenue:.2f} total net revenue',
                f'Previous period: {previous_metrics["line_items"]} line items, SAR {previous_total_revenue:.2f} total net revenue',
                'Refunds excluded from category mix analysis'
            ],
            'assumptions': [
                'Revenue mix analysis uses line_total_sar excluding refunds',
                'Refunds (is_refund=True) excluded from revenue totals',
                'Category mix percentages calculated as category revenue / total net revenue'
            ],
            'confidence': 0.95
        })

# Finding 5: Channel Mix Analysis (excluding refunds)
if len(analysis_channel_revenue) > 0 and len(previous_channel_revenue) > 0:
    analysis_total_revenue = analysis_metrics['total_revenue']
    previous_total_revenue = previous_metrics['total_revenue']
    
    # Get the top channel by revenue for both periods
    top_channel_analysis = analysis_channel_revenue.idxmax()
    top_channel_previous = previous_channel_revenue.idxmax()
    
    top_revenue_analysis = analysis_channel_revenue[top_channel_analysis]
    top_revenue_previous = previous_channel_revenue[top_channel_previous]
    ...
    if mix_shift != 0:
        findings.append({
            'title': 'Channel Revenue Mix Shift Week-over-Week',
            'claim': f'Top channel ({top_channel_analysis}) revenue mix shifted from {previous_mix_pct:.1f}% to {analysis_mix_pct:.1f}% ({mix_shift:+.1f} percentage points)',
            ...
            'coverage_notes': [
                f'Analysis period: {analysis_metrics["line_items"]} line items, SAR {analysis_total_revenue:.2f} total net revenue',
                f'Previous period: {previous_metrics["line_items"]} line items, SAR {previous_total_revenue:.2f} total net revenue',
                'Refunds excluded from channel mix analysis'
            ],
            'assumptions': [
                'Revenue mix analysis uses line_total_sar excluding refunds',
                'Refunds (is_refund=True) excluded from revenue totals',
                'Channel mix percentages calculated as channel revenue / total net revenue'
            ],
            'confidence': 0.95
        })
...
output = {
    ...
    'findings': findings[:5]  # Return at most 5 findings
}
