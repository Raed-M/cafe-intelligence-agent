...
analysis_start = pd.Timestamp('2026-01-12', tz=tz)
analysis_end = pd.Timestamp('2026-01-19', tz=tz)
previous_start = pd.Timestamp('2026-01-05', tz=tz)
previous_end = pd.Timestamp('2026-01-12', tz=tz)
...
pos_analysis['hour'] = pos_analysis['timestamp'].dt.hour
hourly_transactions = pos_analysis[~pos_analysis['is_refund']].groupby('hour')['transaction_id'].nunique()
hourly_revenue = pos_analysis[~pos_analysis['is_refund']].groupby('hour')['line_total_sar'].sum()

# Analyze by day of week
pos_analysis['day_of_week'] = pos_analysis['timestamp'].dt.day_name()
daily_transactions = pos_analysis[~pos_analysis['is_refund']].groupby('day_of_week')['transaction_id'].nunique()
daily_revenue = pos_analysis[~pos_analysis['is_refund']].groupby('day_of_week')['line_total_sar'].sum()
