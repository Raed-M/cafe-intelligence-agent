...
labour_cost_previous = staff_df[(staff_df['date'] >= previous_start.normalize()) & (staff_df['date'] < previous_end.normalize())]['labour_cost_sar'].sum()

# Calculate staff hours
staff_hours_analysis = staff_analysis['computed_duration_hours'].sum()
staff_hours_previous = staff_df[(staff_df['date'] >= previous_start.normalize()) & (staff_df['date'] < previous_end.normalize())]['computed_duration_hours'].sum()
...
if labour_cost_per_transaction_analysis is not None and labour_cost_per_transaction_previous is not None:
    ...
    finding2 = {
        ...
        "coverage_notes": [
            ...
            f"Previous period: {int(staff_df[(staff_df['date'] >= previous_start.normalize()) & (staff_df['date'] < previous_end.normalize())]['employee_id'].nunique())} employees",
