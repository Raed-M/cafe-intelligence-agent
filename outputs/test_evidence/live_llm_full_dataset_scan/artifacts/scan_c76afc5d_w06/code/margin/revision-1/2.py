...
pos_df['timestamp_dt'] = pd.to_datetime(pos_df['timestamp'], utc=True)
inventory_df['week_starting_dt'] = pd.to_datetime(inventory_df['week_starting'], utc=True)
emails_df['date_dt'] = pd.to_datetime(emails_df['date'], utc=True)
emails_df['effective_date_dt'] = pd.to_datetime(emails_df['effective_date'], errors='coerce', utc=True)
...
if len(emails_df) > 0:
    ...
    if len(price_changes) > 0:
        ...
        email_date_str = pd.Timestamp(latest_change['date_dt']).isoformat() if pd.notna(latest_change['date_dt']) else str(latest_change['date'])
        effective_date_str = pd.Timestamp(latest_change['effective_date_dt']).isoformat() if pd.notna(latest_change['effective_date_dt']) else str(latest_change['effective_date'])
        
        finding_3 = {
            "title": "Supplier Price Change Evidence",
            "claim": f"Email evidence documents a price change for {latest_change['entity_or_ingredient']} effective {effective_date_str}: old price {latest_change['old_price']} {latest_change['currency']}/{latest_change['unit']} → new price {latest_change['new_price']} {latest_change['currency']}/{latest_change['unit']}, representing a {latest_change['price_change_pct']:.2f}% change.",
            ...
            "metrics": {
                "ingredient_or_entity": {
                    ...
                    "numerator": None,
                    "denominator": None,
                    "period_start": email_date_str,
                    "period_end": effective_date_str
                },
                "old_price": {
                ...
                "new_price": {
                    ...
                    "denominator": None,
                    "period_start": effective_date_str,
                    "period_end": analysis_end
                },
                "price_change_sar": {
