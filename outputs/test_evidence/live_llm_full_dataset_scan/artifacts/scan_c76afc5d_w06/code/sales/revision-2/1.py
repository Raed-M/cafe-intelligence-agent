...
if abs(aov_pct_change) >= 2.0:
    findings.append({
        ...
        "claim": f"Average order value increased from {previous_aov:.2f} SAR (week of {previous_start.date()}) to {analysis_aov:.2f} SAR (week of {analysis_start.date()}), a change of {aov_change:.2f} SAR ({aov_pct_change:.2f}%).",
...
if abs(cat_metrics['pct_change']) >= 5.0:
    findings.append({
        ...
        "claim": f"Revenue from {cat_name} category decreased from {cat_metrics['previous']:.2f} SAR (week of {previous_start.date()}) to {cat_metrics['current']:.2f} SAR (week of {analysis_start.date()}), a change of {cat_metrics['change']:.2f} SAR ({cat_metrics['pct_change']:.2f}%).",
...
analysis_with_menu = analysis_with_menu[
    ((analysis_with_menu['launch_date'].isna()) | (pd.to_datetime(analysis_with_menu['launch_date']) <= analysis_end)) &
    ((analysis_with_menu['retire_date'].isna()) | (pd.to_datetime(analysis_with_menu['retire_date']) > analysis_start))
]
previous_with_menu = previous_with_menu[
    ((previous_with_menu['launch_date'].isna()) | (pd.to_datetime(previous_with_menu['launch_date']) <= previous_end)) &
    ((previous_with_menu['retire_date'].isna()) | (pd.to_datetime(previous_with_menu['retire_date']) > previous_start))
]
