...
staff_df['shift_start'] = pd.to_datetime(staff_df['shift_start'], format='mixed')
staff_df['shift_end'] = pd.to_datetime(staff_df['shift_end'], format='mixed')
...
# Filter traffic for analysis period - convert to datetime for comparison
...
traffic_previous = traffic_df[
    (traffic_df['date'] >= pd.Timestamp(previous_start.date())) & 
    (traffic_df['date'] < pd.Timestamp(previous_end.date()))
].copy()
...
staff_analysis = staff_df[
    (staff_df['date'] >= pd.Timestamp(analysis_start.date())) & 
    (staff_df['date'] < pd.Timestamp(analysis_end.date()))
].copy()
...
staff_previous = staff_df[
    (staff_df['date'] >= pd.Timestamp(previous_start.date())) & 
    (staff_df['date'] < pd.Timestamp(previous_end.date()))
].copy()
