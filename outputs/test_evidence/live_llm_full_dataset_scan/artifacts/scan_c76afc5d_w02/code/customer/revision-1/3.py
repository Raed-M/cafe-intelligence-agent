...
# Convert reviews date to datetime and make timezone-naive for comparison
reviews_df['date'] = pd.to_datetime(reviews_df['date'])
# Remove timezone info if present to match the comparison
if reviews_df['date'].dt.tz is not None:
    reviews_df['date'] = reviews_df['date'].dt.tz_localize(None)

# Also make analysis dates timezone-naive for comparison
analysis_start_dt = analysis_start_dt.tz_localize(None) if analysis_start_dt.tz is not None else analysis_start_dt
analysis_end_dt = analysis_end_dt.tz_localize(None) if analysis_end_dt.tz is not None else analysis_end_dt
...
if len(analysis_reviews) > 0:
    ...
    finding1 = {
        ...
        "claim": f"During the analysis period (2026-01-19 to 2026-01-26), {int(len(analysis_reviews))} reviews were collected with an average rating of {float(avg_rating):.2f}. The reviews span {int(len(language_counts))} languages and {int(len(source_counts))} sources.",
        ...
        "source_names": sorted([str(s) for s in source_counts.index.tolist()]),
        "sample_size": int(len(analysis_reviews)),
        "coverage_notes": [
            f"Analysis period: 2026-01-19 to 2026-01-26 (7 days)",
            f"Total reviews in dataset: {int(len(reviews_df))}",
            f"Reviews in analysis period: {int(len(analysis_reviews))}",
            f"Language distribution: {dict((str(k), int(v)) for k, v in language_counts.items())}",
            f"Source distribution: {dict((str(k), int(v)) for k, v in source_counts.items())}"
        ],
        "assumptions": [
...
if len(analysis_reviews) > 0:
    ...
    if len(positive_with_text) > 0 or len(negative_with_text) > 0:
        ...
        finding2 = {
            ...
            "claim": f"Of {int(len(analysis_reviews))} reviews, {int(len(positive_reviews))} are positive (rating ≥4), {int(len(negative_reviews))} are negative (rating <3), and {int(len(analysis_reviews) - len(positive_reviews) - len(negative_reviews))} are neutral (rating=3). Positive reviews comprise {int(len(positive_with_text))} with substantive text.",
            ...
            "metrics": {
                ...
                "neutral_reviews": {
                    "value": int(len(analysis_reviews) - len(positive_reviews) - len(negative_reviews)),
                    "unit": "count",
                    "numerator": int(len(analysis_reviews) - len(positive_reviews) - len(negative_reviews)),
                    "denominator": int(len(analysis_reviews)),
                    "period_start": analysis_start,
                    "period_end": analysis_end
                },
                "positive_reviews_with_text": {
                    "value": int(len(positive_with_text)),
                    "unit": "count",
                    "numerator": int(len(positive_with_text)),
                    "denominator": int(len(positive_reviews)) if len(positive_reviews) > 0 else None,
                    "period_start": analysis_start,
                    "period_end": analysis_end
                },
                "negative_reviews_with_text": {
                    "value": int(len(negative_with_text)),
                    "unit": "count",
                    "numerator": int(len(negative_with_text)),
                    "denominator": int(len(negative_reviews)) if len(negative_reviews) > 0 else None,
                    "period_start": analysis_start,
                    "period_end": analysis_end
                }
            },
            "source_names": sorted([str(s) for s in analysis_reviews['source'].unique().tolist()]),
            "sample_size": int(len(analysis_reviews)),
            "coverage_notes": [
                f"Sentiment classification based on rating thresholds: positive (≥4), neutral (=3), negative (<3)",
                f"Language coverage: {dict((str(k), int(v)) for k, v in language_counts.items())}",
                f"Positive reviews with substantive text: {int(len(positive_with_text))}",
                f"Negative reviews with substantive text: {int(len(negative_with_text))}"
            ],
            "assumptions": [
...
if len(analysis_reviews) > 0:
    ...
    finding3 = {
        ...
        "claim": f"During the analysis period, reviews were collected from {int(len(source_distribution))} sources: {', '.join(sorted([str(s) for s in source_distribution.index.tolist()]))}. The distribution shows {dict((str(k), int(v)) for k, v in source_distribution.items())}.",
    ...
    for source, count in source_distribution.items():
        source_key = f"{str(source)}_review_count"
    ...
    finding3["source_names"] = sorted([str(s) for s in source_distribution.index.tolist()])
    finding3["sample_size"] = int(len(analysis_reviews))
    finding3["coverage_notes"] = [
        f"Total reviews analyzed: {int(len(analysis_reviews))}",
        f"Source distribution: {dict((str(k), int(v)) for k, v in source_distribution.items())}",
