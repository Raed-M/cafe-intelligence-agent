...
# Ensure timezone-aware comparison: convert analysis dates to UTC and strip timezone for comparison with tz-naive data
# If data is tz-naive, convert analysis dates to tz-naive
if pos_df['timestamp'].dt.tz is None:
    analysis_start_cmp = analysis_start.replace(tzinfo=None)
    analysis_end_cmp = analysis_end.replace(tzinfo=None)
else:
    analysis_start_cmp = analysis_start
    analysis_end_cmp = analysis_end

if inventory_df['week_starting'].dt.tz is None:
    analysis_start_inv = analysis_start.replace(tzinfo=None)
    analysis_end_inv = analysis_end.replace(tzinfo=None)
else:
    analysis_start_inv = analysis_start
    analysis_end_inv = analysis_end

# Filter POS data for analysis period
pos_analysis = pos_df[(pos_df['timestamp'] >= analysis_start_cmp) & (pos_df['timestamp'] < analysis_end_cmp)].copy()

# Filter inventory for analysis period
inventory_analysis = inventory_df[(inventory_df['week_starting'] >= analysis_start_inv) & (inventory_df['week_starting'] < analysis_end_inv)].copy()
    ...
    finding_1 = {
        ...
        "metrics": {
            ...
            "gross_margin_percentage": {
                ...
                "unit": "%",
                "numerator": round(top_item['gross_profit'], 1),
                "denominator": round(top_item['net_revenue'], 1),
                "period_start": "2026-01-19T00:00:00+03:00",
                "period_end": "2026-01-26T00:00:00+03:00"
            },
            "units_sold": {
...
if len(inventory_with_waste) > 0:
    ...
    finding_2 = {
        ...
        "claim": f"Waste cost of {total_waste_cost:.1f} SAR from {int(total_waste_units)} units across {len(inventory_with_waste)} SKUs with recorded waste represents {waste_percentage:.2f}% of total product cost (COGS + waste) for these items.",
        ...
        "coverage_notes": [
            ...
            "Analysis period: 2026-01-19 to 2026-01-26",
            "Percentage applies only to the subset of SKUs with recorded waste"
        ],
        "assumptions": [
            ...
            "Denominator (total_product_cost) = COGS + waste_cost for items with recorded waste"
        ],
        "confidence": 0.90
    }
