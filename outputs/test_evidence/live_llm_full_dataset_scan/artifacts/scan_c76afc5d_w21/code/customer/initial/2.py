...
# Convert to datetime for filtering - ensure timezone awareness
...
# If reviews_df['date'] is tz-naive, make it tz-aware; if tz-aware, keep it
if reviews_df['date'].dt.tz is None:
    # tz-naive: localize to UTC then convert to +03:00
    reviews_df['date'] = reviews_df['date'].dt.tz_localize('UTC').dt.tz_convert('+03:00')
else:
    # tz-aware: ensure it's in the same timezone for comparison
    reviews_df['date'] = reviews_df['date'].dt.tz_convert('+03:00')

# Make comparison datetimes tz-aware in the same timezone
