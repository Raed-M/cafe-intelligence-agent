...
if len(reviews_analysis) > 0:
    ...
    sources_list = sorted(source_counts.index.tolist())
    ...
    finding_1 = {
        ...
        "claim": f"During the analysis period ({analysis_start} to {analysis_end}), {int(len(reviews_analysis))} reviews were collected with an average rating of {float(avg_rating):.2f} out of 5. Rating distribution: {dict((int(k), int(v)) for k, v in rating_counts.items())}. Sources: {dict((str(k), int(v)) for k, v in source_counts.items())}. Languages: {dict((str(k), int(v)) for k, v in language_counts.items())}.",
        ...
        "coverage_notes": [
            ...
            f"Reviews in analysis period: {int(len(reviews_analysis))}",
...
if len(reviews_analysis) > 0:
    ...
    finding_2 = {
        ...
        "claim": f"Among {int(len(reviews_analysis))} reviews in the analysis period, sentiment distribution is: {dict((str(k), int(v)) for k, v in sentiment_counts.items())}. English reviews ({int(len(english_reviews))}): {dict((str(k), int(v)) for k, v in en_sentiment.items())}. Arabic reviews ({int(len(arabic_reviews))}): {dict((str(k), int(v)) for k, v in ar_sentiment.items())}.",
        ...
        "metrics": {
            ...
            "english_reviews_count": {
                ...
                "unit": "count",
                "numerator": int(len(english_reviews)),
                "denominator": int(len(reviews_analysis)),
                "period_start": analysis_start,
                "period_end": analysis_end
            },
            "arabic_reviews_count": {
                ...
                "unit": "count",
                "numerator": int(len(arabic_reviews)),
                "denominator": int(len(reviews_analysis)),
                "period_start": analysis_start,
                "period_end": analysis_end
            }
        ...
        "coverage_notes": [
            ...
            f"English reviews: {int(len(english_reviews))}",
            f"Arabic reviews: {int(len(arabic_reviews))}",
...
if len(reviews_analysis) > 0:
    ...
    finding_3 = {
        ...
        "claim": f"Of {int(len(reviews_analysis))} reviews in the analysis period, {int(len(non_empty_reviews))} contain text content and {int(empty_reviews)} are empty or missing text.",
        "finding_type": "data_quality_metric",
        "metrics": {
            "reviews_with_text": {
                "value": int(len(non_empty_reviews)),
                "unit": "count",
                "numerator": int(len(non_empty_reviews)),
                "denominator": int(len(reviews_analysis)),
                "period_start": analysis_start,
                "period_end": analysis_end
            },
            "reviews_without_text": {
                "value": int(empty_reviews),
                "unit": "count",
                "numerator": int(empty_reviews),
                "denominator": int(len(reviews_analysis)),
                "period_start": analysis_start,
                "period_end": analysis_end
            },
            "text_coverage_percentage": {
                "value": float(round((len(non_empty_reviews) / len(reviews_analysis)) * 100, 1)),
                "unit": "percent",
                "numerator": int(len(non_empty_reviews)),
                "denominator": int(len(reviews_analysis)),
                "period_start": analysis_start,
        ...
        "coverage_notes": [
            f"Analysis period: {analysis_start} to {analysis_end}",
            f"Total reviews: {int(len(reviews_analysis))}",
            f"Reviews with text: {int(len(non_empty_reviews))}",
            f"Reviews without text: {int(empty_reviews)}",
