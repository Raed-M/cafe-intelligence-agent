...
findings.append({
    ...
    "coverage_notes": [
        ...
        "Transaction count based on unique transaction_id values (includes all transactions including refunds)",
        "All transactions in date range included; no filtering by refund status for transaction count"
    ],
    "assumptions": [
        "All transactions in POS data are valid and complete",
        "line_total_sar represents net revenue including refunds as negative values",
        "Unique transaction_id represents a basket/transaction",
        "Timestamp filtering uses UTC+3 timezone as provided",
        "Transaction count includes all transactions regardless of refund status"
    ],
    "confidence": 0.95
})
...
if len(analysis_category_revenue) > 0 and len(previous_category_revenue) > 0:
    ...
    analysis_top_rev = float(analysis_category_revenue.iloc[0])
    previous_top_rev = float(previous_category_revenue.get(top_category, 0))
    ...
    analysis_top_items = int(len(analysis_with_category[analysis_with_category['category_menu'] == top_category]))
    previous_top_items = int(len(previous_with_category[previous_with_category['category_menu'] == top_category]))
    ...
    findings.append({
        ...
        "coverage_notes": [
            ...
            "Refunds included in net revenue as negative line_total_sar values",
            "All transactions in date range included regardless of refund status"
        ],
        "assumptions": [
            ...
            "Refunds included in revenue calculations"
        ],
        "confidence": 0.92
    })
...
if len(analysis_channel) > 0:
    ...
    analysis_top_channel_rev = float(analysis_channel.iloc[0])
    previous_top_channel_rev = float(previous_channel.get(top_channel, 0))
    
    analysis_top_channel_baskets = int(analysis_channel_baskets.get(top_channel, 0))
    previous_top_channel_baskets = int(previous_channel_baskets.get(top_channel, 0))
