...
# Convert reviews date to datetime and make timezone-aware if needed
reviews_df['date'] = pd.to_datetime(reviews_df['date'])

# If analysis dates are timezone-aware and review dates are naive, localize them
if analysis_start_dt.tz is not None and reviews_df['date'].dt.tz is None:
    # Assume reviews are in the same timezone as analysis period
    reviews_df['date'] = reviews_df['date'].dt.tz_localize('UTC').dt.tz_convert(analysis_start_dt.tz)
elif analysis_start_dt.tz is None and reviews_df['date'].dt.tz is not None:
    # Remove timezone from review dates
    reviews_df['date'] = reviews_df['date'].dt.tz_localize(None)
elif analysis_start_dt.tz is not None and reviews_df['date'].dt.tz is not None:
