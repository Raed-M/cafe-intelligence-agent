...
# Handle timezone awareness: convert to UTC if needed for comparison
if reviews_df['date'].dt.tz is None:
    # If reviews_df dates are naive, make them aware with UTC
    reviews_df['date'] = reviews_df['date'].dt.tz_localize('UTC')

# Ensure comparison datetimes are in UTC
if analysis_start_dt.tz is None:
    analysis_start_dt = analysis_start_dt.tz_localize('UTC')
else:
    analysis_start_dt = analysis_start_dt.tz_convert('UTC')

if analysis_end_dt.tz is None:
    analysis_end_dt = analysis_end_dt.tz_localize('UTC')
else:
...
if len(analysis_reviews) > 0:
    ...
    finding1 = {
        ...
        "claim": f"During the analysis period (2026-07-06 to 2026-07-13), customers provided {len(analysis_reviews)} reviews with an average rating of {avg_rating:.2f} out of 5. The majority of reviews ({int(sentiment_counts.get('positive', 0))} out of {len(analysis_reviews)}) were positive (rating ≥4).",
...
if len(analysis_reviews) > 0:
    ...
    finding2 = {
        ...
        "claim": f"Among {len(analysis_reviews)} reviews, {int(language_counts.get('en', 0))} were in English and {int(language_counts.get('ar', 0))} were in Arabic. The most frequently mentioned topics in reviews with text ({len(reviews_with_text)} reviews) were quality ({int(top_topics[0][1])} mentions), service ({int(top_topics[1][1])} mentions), and taste ({int(top_topics[2][1])} mentions).",
        ...
        "metrics": {
            ...
            "quality_mentions": {
                "value": int(topics['quality']),
                "unit": "count",
                "numerator": int(topics['quality']),
                "denominator": int(len(reviews_with_text)) if len(reviews_with_text) > 0 else None,
                "period_start": analysis_start,
                "period_end": analysis_end
            },
            "service_mentions": {
                "value": int(topics['service']),
                "unit": "count",
                "numerator": int(topics['service']),
                "denominator": int(len(reviews_with_text)) if len(reviews_with_text) > 0 else None,
                "period_start": analysis_start,
                "period_end": analysis_end
            }
    ...
    if len(language_ratings) > 1:
        ...
        if en_avg is not None and ar_avg is not None:
            finding3 = {
                ...
                "metrics": {
                    "english_avg_rating": {
                        "value": float(round(en_avg, 2)),
                        "unit": "stars",
                        "numerator": float(round(analysis_reviews[analysis_reviews['language'] == 'en']['rating'].sum(), 2)),
                        "denominator": int(language_ratings.loc['en', 'count']),
                        "period_start": analysis_start,
                        "period_end": analysis_end
                    },
                    "arabic_avg_rating": {
                        "value": float(round(ar_avg, 2)),
                        "unit": "stars",
                        "numerator": float(round(analysis_reviews[analysis_reviews['language'] == 'ar']['rating'].sum(), 2)),
