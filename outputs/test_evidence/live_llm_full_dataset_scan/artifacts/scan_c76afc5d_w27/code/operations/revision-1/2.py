...
# Define periods (Asia/Riyadh is UTC+3)
# Using fixed offset instead of pytz
utc_plus_3 = datetime.now().astimezone().tzinfo
analysis_start = datetime(2026, 7, 13, 0, 0, 0)
analysis_end = datetime(2026, 7, 20, 0, 0, 0)
prev_start = datetime(2026, 7, 6, 0, 0, 0)
prev_end = datetime(2026, 7, 13, 0, 0, 0)

# Convert timestamps to timezone-aware (assuming UTC+3 for Riyadh)
# Create timezone offset for UTC+3
from datetime import timezone, timedelta as td
riyadh_tz = timezone(td(hours=3))

pos_df['timestamp'] = pd.to_datetime(pos_df['timestamp'], utc=True).dt.tz_convert(riyadh_tz)
traffic_df['date'] = pd.to_datetime(traffic_df['date']).dt.tz_localize(riyadh_tz)
staff_df['date'] = pd.to_datetime(staff_df['date']).dt.tz_localize(riyadh_tz)
staff_df['shift_start'] = pd.to_datetime(staff_df['shift_start'], utc=True).dt.tz_convert(riyadh_tz)
staff_df['shift_end'] = pd.to_datetime(staff_df['shift_end'], utc=True).dt.tz_convert(riyadh_tz)
inventory_df['week_starting'] = pd.to_datetime(inventory_df['week_starting']).dt.tz_localize(riyadh_tz)

# Localize period boundaries
analysis_start = analysis_start.replace(tzinfo=riyadh_tz)
analysis_end = analysis_end.replace(tzinfo=riyadh_tz)
prev_start = prev_start.replace(tzinfo=riyadh_tz)
prev_end = prev_end.replace(tzinfo=riyadh_tz)
