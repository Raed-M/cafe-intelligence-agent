        ...
        findings.append({
            ...
            'metrics': {
                'observed_daily_revenue_sar': {
                    ...
                    'numerator': None,
                    'denominator': None,
                    'period_start': top_anomaly_rev['date'].strftime('%Y-%m-%d'),
                    'period_end': (pd.to_datetime(top_anomaly_rev['date']) + timedelta(days=1)).strftime('%Y-%m-%d')
                },
    ...
    if baseline_traffic_std > 0 and len(baseline_traffic_flat) >= 20:
        ...
        if len(anomaly_traffic) > 0:
            ...
            findings.append({
                ...
                'claim': f"Hourly door count on {top_anomaly_traffic['date']} at hour {int(top_anomaly_traffic['hour'])} was {int(top_anomaly_traffic['door_count'])} visitors, {abs(top_anomaly_traffic['z_score']):.2f} standard deviations from baseline mean of {baseline_traffic_mean:.1f}.",
                ...
                'metrics': {
                    'observed_hourly_door_count': {
                        ...
                        'numerator': None,
                        'denominator': None,
                        'period_start': f"{top_anomaly_traffic['date']}T{int(top_anomaly_traffic['hour']):02d}:00:00+03:00",
                        'period_end': f"{top_anomaly_traffic['date']}T{int(top_anomaly_traffic['hour'])+1:02d}:00:00+03:00"
                    },
...
reviews_analysis = reviews_df[
    (reviews_df['date'] >= analysis_start.date()) & 
    (reviews_df['date'] < analysis_end.date())
].copy()
...
if len(reviews_analysis) > 0:
    ...
    for period_start, period_end in trailing_periods:
        reviews_baseline = reviews_df[
            (reviews_df['date'] >= period_start.date()) & 
            (reviews_df['date'] < period_end.date())
        ].copy()
        ...
        if baseline_rating_std > 0 and len(baseline_rating_flat) >= 15:
            ...
            if len(anomaly_rating) > 0:
                ...
                findings.append({
                    ...
                    'claim': f"Average rating on {top_anomaly_rating['date']} was {top_anomaly_rating['avg_rating']:.2f} ({int(top_anomaly_rating['count'])} reviews), {abs(top_anomaly_rating['z_score']):.2f} standard deviations from baseline mean of {baseline_rating_mean:.2f}.",
                    ...
                    'metrics': {
                        'observed_daily_avg_rating': {
                            ...
                            'numerator': None,
                            'denominator': None,
                            'period_start': str(top_anomaly_rating['date']),
                            'period_end': str(top_anomaly_rating['date'] + timedelta(days=1))
                        },
