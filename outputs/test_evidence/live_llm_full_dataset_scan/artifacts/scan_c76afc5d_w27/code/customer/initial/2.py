...
if total_reviews > 0:
    finding1 = {
        ...
        "claim": f"Average rating is {avg_rating:.2f}/5 across {total_reviews} reviews during the analysis period, with {convert_to_native(sentiment_distribution.get('positive', 0))} positive, {convert_to_native(sentiment_distribution.get('neutral', 0))} neutral, and {convert_to_native(sentiment_distribution.get('negative', 0))} negative reviews.",
        "finding_type": "voice_of_customer",
        "metrics": {
            "average_rating": {
                "value": round(float(avg_rating), 2),
                "unit": "stars",
                "numerator": convert_to_native(total_reviews),
            ...
            "positive_reviews": {
                "value": convert_to_native(sentiment_distribution.get('positive', 0)),
                "unit": "count",
                "numerator": convert_to_native(sentiment_distribution.get('positive', 0)),
                "denominator": convert_to_native(total_reviews),
                "period_start": "2026-07-13T00:00:00+03:00",
                "period_end": "2026-07-20T00:00:00+03:00"
            },
            "negative_reviews": {
                "value": convert_to_native(sentiment_distribution.get('negative', 0)),
                "unit": "count",
                "numerator": convert_to_native(sentiment_distribution.get('negative', 0)),
                "denominator": convert_to_native(total_reviews),
                "period_start": "2026-07-13T00:00:00+03:00",
                "period_end": "2026-07-20T00:00:00+03:00"
            }
        ...
        "coverage_notes": [
            ...
            f"Total reviews in period: {convert_to_native(total_reviews)}",
            f"Language distribution: {dict((k, convert_to_native(v)) for k, v in language_distribution.items())}",
            f"Platform distribution: {dict((k, convert_to_native(v)) for k, v in source_distribution.items())}"
        ],
        "assumptions": [
...
if len(topic_counts) > 0:
    ...
    finding2 = {
        ...
        "claim": f"The most discussed topics in customer reviews are {', '.join([f'{topic} ({convert_to_native(count)} mentions)' for topic, count in top_topics])}.",
        "finding_type": "voice_of_customer",
        "metrics": {
            "top_topic_1": {
...
if len(source_distribution) > 0:
    ...
    if 'positive' in platform_sentiments.columns:
        ...
        finding3 = {
            ...
            "claim": f"Platform '{best_platform}' has the highest proportion of positive reviews with {convert_to_native(best_platform_positive)} positive out of {convert_to_native(best_platform_total)} total reviews.",
            "finding_type": "voice_of_customer",
            "metrics": {
                "best_platform": {
                    "value": best_platform,
                    "unit": "platform",
                    "numerator": convert_to_native(best_platform_positive),
                    "denominator": convert_to_native(best_platform_total),
                    "period_start": "2026-07-13T00:00:00+03:00",
                    "period_end": "2026-07-20T00:00:00+03:00"
                },
                "positive_ratio": {
                    "value": round(float(best_platform_positive) / float(best_platform_total), 2),
                    "unit": "ratio",
                    "numerator": convert_to_native(best_platform_positive),
                    "denominator": convert_to_native(best_platform_total),
                    "period_start": "2026-07-13T00:00:00+03:00",
