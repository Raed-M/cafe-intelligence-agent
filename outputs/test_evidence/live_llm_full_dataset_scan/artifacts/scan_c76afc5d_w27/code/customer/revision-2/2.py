...
if len(reviews_period) > 0:
    ...
    finding_1 = {
        ...
        "claim": f"During the analysis period (2026-07-13 to 2026-07-20), the cafe received {int(len(reviews_period))} reviews with an average rating of {avg_rating:.2f} out of 5.0.",
        "finding_type": "voice_of_customer",
        "metrics": {
            "total_reviews": {
                "value": int(len(reviews_period)),
                "unit": "count",
                "numerator": int(len(reviews_period)),
                "denominator": None,
            ...
            "average_rating": {
                ...
                "unit": "stars",
                "numerator": round(float(reviews_period['rating'].sum()), 2),
                "denominator": int(len(reviews_period)),
                "period_start": analysis_start,
                "period_end": analysis_end
            }
        },
        "source_names": ["reviews"],
        "sample_size": int(len(reviews_period)),
        "coverage_notes": [
            f"Analysis period: 2026-07-13 to 2026-07-20 (7 days)",
            f"Total reviews in artifact: {int(len(reviews_df))}",
            f"Reviews in analysis period: {int(len(reviews_period))}",
            f"Language distribution: {dict((k, int(v)) for k, v in reviews_period['language'].value_counts().items())}"
        ],
        "assumptions": [
...
if len(reviews_period) > 0:
    ...
    for platform in reviews_period['source'].unique():
        ...
        positive_count = int((platform_reviews['rating'] >= 4).sum())
        total_count = int(len(platform_reviews))
    ...
    # Build platform distribution string for coverage notes
    platform_dist_str = ", ".join([f"{platform}: {int(count)}" for platform, count in platform_counts.items()])
    
    finding_2 = {
        "title": "Platform with Highest Positive Review Ratio",
        "claim": f"Among the {int(len(platform_counts))} platforms analyzed, '{best_platform_name}' has the highest proportion of positive reviews, with {best_platform_data['positive']} positive reviews out of {best_platform_data['total']} total reviews on the {best_platform_name} platform ({best_platform_data['ratio']*100:.1f}% positive ratio).",
        "finding_type": "voice_of_customer",
        "metrics": {
            "best_platform": {
        ...
        "coverage_notes": [
            ...
            f"Platform distribution: {platform_dist_str}",
...
if len(reviews_period) > 0:
    ...
    # Build language distribution string for coverage notes
    lang_dist_str = ", ".join([f"{lang}: {int(count)}" for lang, count in language_counts.items()])
    
    finding_3 = {
        "title": "Review Language Distribution",
        "claim": f"During the analysis period, reviews were submitted in {int(len(language_counts))} language(s). {language_counts.index[0]} reviews comprise {int(language_counts.iloc[0])} out of {int(len(reviews_period))} reviews ({language_counts.iloc[0]/len(reviews_period)*100:.1f}%).",
        "finding_type": "voice_of_customer",
        "metrics": {
            "total_languages": {
                "value": int(len(language_counts)),
                "unit": "count",
                "numerator": int(len(language_counts)),
                "denominator": None,
                "period_start": analysis_start,
                "period_end": analysis_end
            },
            "primary_language": {
                "value": str(language_counts.index[0]),
                "unit": None,
                "numerator": None,
                "denominator": None,
                "period_start": analysis_start,
            ...
            "primary_language_count": {
                "value": int(language_counts.iloc[0]),
                "unit": "count",
                "numerator": int(language_counts.iloc[0]),
                "denominator": int(len(reviews_period)),
                "period_start": analysis_start,
                "period_end": analysis_end
            }
        ...
        "coverage_notes": [
            f"Analysis period: 2026-07-13 to 2026-07-20",
            f"Language distribution: {lang_dist_str}",
            f"Total reviews analyzed: {int(len(reviews_period))}"
        ],
        "assumptions": [
